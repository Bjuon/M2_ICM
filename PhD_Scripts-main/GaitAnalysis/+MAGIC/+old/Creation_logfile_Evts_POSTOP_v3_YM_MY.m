...
warning(['Check Left_Foot_Off : '  filename])
    ...
    warning(['Check Left_Foot_Strike : '  filename])
                                                       ...
                                                       %herepbm check si  meme longueur
if length(DATA.FO_R) ~= length(DATA.FC_R)
    fprintf(2,['Check Right_Cycle : '  filename])
end

if length(DATA.FO_L) ~= length(DATA.FC_L)
    fprintf(2,['Check Left_Cycle : '  filename])
end
...
disp(['Check if FOG exist et si oui, check nomenclature : '  filename ])
    ...
    else isfield(Ev,'General_Start_FOG')
    DATA.End_FOG  = NaN;
    disp(['Check if FOG exist et si oui, check nomenclature : ' filename])
    end
