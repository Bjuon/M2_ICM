function [Donnes EMG] = pretraitement_dataAPA_v4(files,dossier,b_c)
...
%             fin = length(t);
    ...
    CP_filt = filtrage(CP,'fir',50,45,Freq_vid); %%%% A changer?
    ...
    if isfield(DATA,'ANLG')
        ...
        if ~isnan(signal)
            ...
            try
                TR_ind = find(signal>0.25,1,'first');
...
%             tags_fog = DATA.events.noms(tags_fog);
            evts = DATA.events.temps(~tags_fog);
        end
        pause(1);
        
        if length(evts)<5
            disp('...Pas d''évènements du pas détectés ');
        else
