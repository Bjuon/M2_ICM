function [Pr,d,Loc] = projection_points_courbe_D(Ref,Rec) ;
%
...
%
if ~isfield(Rec,'tangent') ;
    Rec = calcul_repere_frenet(Rec) ;
end
...
%
A = Ref.coord * Rec.tangent' - ones(size(Ref.coord,1),1) * (sum(Rec.tangent .* Rec.coord,2)') ;
%
...
A.localisation = zeros(length(Rec.coord),2) ;
...
for t = min(A.Nb_int):max(A.Nb_int) ;
    ...
    if ~isempty(qui) ;
        ...
        for y = 1:t ;
            ...
            Va = Rec.tangent(Pt_Rec,:) ;
            Vp = Rec.tangent(Pt_Rec,:) ;
            ...
            if ~isempty(U) ;
                ...
                A.localisation(Pt_Rec(lU),[2*y-1,2*y]) = [Pt_pre(lU),Pt_post(lU)] ;
            end
        end
    end
end
...
%
I = find(A.localisation(:,[2:2:end]) == 0) ;
A.dist(I) = NaN ;
% ---> Recherche des distances minimales
%
if size(A.localisation,2)/2 ~= 1 ; % Il y a plusieurs intersections pour 1 point
    Temp = A.dist' ;
    ...
    for t = 1:length(A.Nb_int) ;
        Temp12([2:2:size(Temp1,1)],t) = Temp1(2*ordre(:,t),t) ;
        Temp12([1:2:size(Temp1,1)-1],t) = Temp1(2*ordre(:,t)-1,t) ;
...
Loc = NaN * ones(size(Rec.coord,1),2) ;
Loc(liste_ok,:) = A.localisation(liste_ok,1:2) ;
%
% Fin de la fonction