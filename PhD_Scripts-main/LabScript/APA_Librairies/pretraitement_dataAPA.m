function [Donnes EMG] = pretraitement_dataAPA(files,dossier)
...
for i = 1:nb_acq
    ...
    fin = round(find(Fres==0,1,'first')) - 1;
    if isempty(fin)
        fin = round(find(Fres(:,3)<50,1,'first'));
    end
    %Filtrage des données PF: filtre à réponse impulsionnel finie d'ordre 30 et de fréquence de coupure 8Hz (Prieto et al. 1996)
    if isempty(DATA.coord)
        Freq_vid = btkGetAnalogFrequency(h);
    end
    ...
    CP_filt = filtrage(CP,'fir',20,30,Freq_vid); %%%% A changer
    ...
    if ind_1>2 % Cas ou l'acqisition commence tardivement avec l'initiation du pas
        ...
        waitfor(errordlg(['Pas d''évènements du pas ' fichier]));
        DATA.events.temps = evts;
    end
    
    Donnes.(acq).tMarkers.HO = evts(1);
    Donnes.(acq).tMarkers.TO = evts(2);
    Donnes.(acq).tMarkers.FC1 = evts(3);
    Donnes.(acq).tMarkers.FO2 = evts(4);
    Donnes.(acq).tMarkers.FC2 = evts(5);
    ...
    Donnes.(acq).Acc_Z = filtrage(Acc(l,3),'fir',30,8,Freq_vid); % Accélération verticale (obtenue grace à la PF)
