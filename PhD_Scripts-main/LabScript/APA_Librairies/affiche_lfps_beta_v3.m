function lfp  = affiche_lfps_beta_v3(listes_acqs)
...
try
    ...
    f=figure('Name',[Subject_data.ID '_0-45Hz']);
    f1=figure('Name',[Subject_data.ID '_55-100Hz']);
    for c=1:N_lfp
        i_check = 0;
        for i=1:N_acq
            try
...
%         area(F_T0(1:80),mean(P_T0(:,1:80,c),1),'Linewidth',1.5,'FaceColor','g','Linestyle','-.'); hold on
        ...
        if comparaison
            ...
            current_tags = [tags{end-1} '-' tags{end}(1:end-1)];
...
%         legend('Before Instruction','Before T0','Whole trial');
    ...
    if strcmp(button,'Oui')
        path = uigetdir(cd,'Choix du dossier pour le stockage des images');
        tags = extract_tags(acquisitions{acqs(1)});
        file_to_print = ['PSD_' tags{1} '_' tags{end-1} '_' tags{end}(1:end-1)];
