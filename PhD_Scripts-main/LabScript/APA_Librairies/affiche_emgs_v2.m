function f  = affiche_emgs_v2(listes_acqs)
%% Fonction d'affichage des données EMG (EMG) dans une nouvelle interface f
global  h_RTA h_RSOL h_LTA h_LSOL list_emg b1 Activation_EMG_percycle multi_emg
...
L = uiextras.VBox( 'Parent', b2);
list_emg = uicontrol( 'Style', 'listbox', 'Parent', L, 'String', listes_acqs,'Callback',@listEMG_Callback);
multi_emg = uicontrol( 'Style', 'radiobutton', 'Parent', L, 'String', 'Affichage multiple','tag','multi_emgs','Value',0);

%Ajout des boutons d'actions pour le traitement
Bs = uiextras.VBox( 'Parent', b2);
uicontrol( 'Style', 'pushbutton', 'Parent', Bs, 'String', 'Editer/Recharger EMG','Callback',@editEMG_Callback);
uicontrol( 'Style', 'pushbutton', 'Parent', Bs, 'String', 'Tibial Antérieur D','Callback',@RTA_Callback);
uicontrol( 'Style', 'pushbutton', 'Parent', Bs, 'String', 'Soléaire D','Callback',@RSOL_Callback);
uicontrol( 'Style', 'pushbutton', 'Parent', Bs, 'String', 'Tibial Antérieur G','Callback',@LTA_Callback);
uicontrol( 'Style', 'pushbutton', 'Parent', Bs, 'String', 'Soléaire G','Callback',@LSOL_Callback);
uicontrol( 'Style', 'pushbutton', 'Parent', Bs,'Tag','Affiche_EMGs_norm','String', 'Calcul/Affiche période(s) d''activations normalisées','Callback',@affiche_activation,'Enable','Off');
uicontrol( 'Style', 'pushbutton', 'Parent', Bs,'Tag','Affiche_EMGs_histo','String', 'Calcul histogramme d''activations normalisées','Callback',@affiche_histo,'Enable','Off');
uicontrol( 'Style', 'pushbutton', 'Parent', Bs,'Tag','Export_EMGs_activation','String', 'Exporter temps et % d''activations normalisées + iEMG','Callback',@export_emgs,'Enable','Off');
...
%% Affichage EMGs
global h_RTA h_LTA h_RSOL h_LSOL EMG list_emg acq_choisie Activation_EMG Corridors_EMG Histogram_EMG Sujet b1 h_marks_T0_emg h_marks_FC1_emg h_marks_FC2_emg h_T0_txt h_FC1_txt h_FC2_txt multi_emg
...
try
    ...
    acq_choisie = contents{get(list_emg,'Value')};
     ...
     %Initialisation des plots/couleurs et marqueurs si Multiplot Off
     axess = findobj('Type','axes','Parent',b1);
     if ~get(multi_emg,'Value')        
     ...
     if ~isfield(Corridors_EMG,acq_choisie) && ~isfield(Histogram_EMG,acq_choisie)
         ...
         EMGs_filtered = abs(TraitementEMG(EMG.(acq_choisie).val,Fs)); % On redresse
         ...
         plot(h_RTA,t,smooth(EMGs_filtered(1:length(t),1)),'Color',c); ylabel(h_RTA,EMG.(acq_choisie).nom(1)); axis(h_RTA,'tight'); %set(h_RTA,'ylim',[-5 5]);
         plot(h_LTA,t,smooth(EMGs_filtered(1:length(t),2)),'Color',c); ylabel(h_LTA,EMG.(acq_choisie).nom(2)); axis(h_LTA,'tight'); %set(h_LTA,'ylim',[-5 5]);
         plot(h_RSOL,t,smooth(EMGs_filtered(1:length(t),4)),'Color',c); ylabel(h_RSOL,EMG.(acq_choisie).nom(4)); axis(h_RSOL,'tight'); %set(h_RSOL,'ylim',[-5 5]);
         plot(h_LSOL,t,smooth(EMGs_filtered(1:length(t),3)),'Color',c); ylabel(h_LSOL,EMG.(acq_choisie).nom(3)); axis(h_LSOL,'tight'); %set(h_LSOL,'ylim',[-5 5]);
        ...
        h_T0_txt = text(Sujet.(acq_choisie).tMarkers.T0,max(EMGs_filtered(:,1))/2,'T0',...
            'VerticalAlignment','middle',...
            'HorizontalAlignment','Right',...
            'FontSize',10,...
            'Parent',h_RTA);
        h_marks_FC1_emg = affiche_marqueurs(Sujet.(acq_choisie).tMarkers.FC1,'-m');
        h_FC1_txt = text(Sujet.(acq_choisie).tMarkers.FC1,max(EMGs_filtered(:,1))/2,'FC1',...
            'VerticalAlignment','middle',...
            'HorizontalAlignment','Right',...
            'FontSize',10,...
            'Parent',h_RTA);
        h_marks_FC2_emg = affiche_marqueurs(Sujet.(acq_choisie).tMarkers.FC2,'-c');
        h_FC2_txt = text(Sujet.(acq_choisie).tMarkers.FC2,max(EMGs_filtered(:,1))/2,'FC2',...
            'VerticalAlignment','middle',...
            'HorizontalAlignment','Right',...
            'FontSize',10,...
            'Parent',h_RTA);
     ...
     elseif isfield(Corridors_EMG,acq_choisie)
         ...
         stdshade(abs(Corridors_EMG.(acq_choisie).(muscles{1})),0.25,c,t,1,h_RTA,1.25,1); ylabel(h_RTA,[muscles{1} ' Z-normalisé']); set(h_RTA,'ylim',[-5 5]); axis(h_RTA,'tight');
         stdshade(abs(Corridors_EMG.(acq_choisie).(muscles{2})),0.25,c,t,1,h_LTA,1.25,1); ylabel(h_LTA,[muscles{2} ' Z-normalisé']); set(h_LTA,'ylim',[-5 5]); axis(h_LTA,'tight');  
         stdshade(abs(Corridors_EMG.(acq_choisie).(muscles{3})),0.25,c,t,1,h_RSOL,1.25,1); ylabel(h_RSOL,[muscles{3} ' Z-normalisé']); set(h_RSOL,'ylim',[-5 5]); axis(h_RSOL,'tight');
         stdshade(abs(Corridors_EMG.(acq_choisie).(muscles{4})),0.25,c,t,1,h_LSOL,1.25,1); ylabel(h_LSOL,[muscles{4} ' Z-normalisé']); set(h_LSOL,'ylim',[-5 5]); axis(h_LSOL,'tight');
