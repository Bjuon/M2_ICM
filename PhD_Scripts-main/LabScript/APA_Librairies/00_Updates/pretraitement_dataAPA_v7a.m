function [Donnes EMG] = pretraitement_dataAPA_v7(files,dossier,b_c)
    ...
    if isfield(DATA,'Analogs')
        ...
        if ~isnan(signal)
            ...
            try
                ...
                Donnes.(acq).tMarkers.TR = TR_ind/DATA.Analogs.Fech;
...
%     pause(0.5);
    ...
    Donnes.(acq).Fech = Freq_ana; %Fréquence d'échantillonage (Analogique)
    Donnes.(acq).Fech_vid = Freq_vid; %Fréquence d'échantillonage (Analogique)
    ...
    if ~isempty(V_CG_Der)
        Donnes.(acq).V_CG_AP_d = V_CG_Der(:,2); % Vitesse du CG (OBassin) AP (dérivation)
        Donnes.(acq).V_CG_Z_d = V_CG_Der(:,3); % Vitesse verticale du CG (OBassin) (dérivation)
    end 
    
    if ~isempty(V_CG_Vic)
        Donnes.(acq).V_CG_Vic_AP_d = V_CG_Vic(:,2); % Vitesse du CG de VICON AP (dérivation)
        Donnes.(acq).V_CG_Vic_Z_d = V_CG_Vic(:,3); % Vitesse verticale du CG de VICON (dérivation)
    end 
    
    Donnes.(acq).Acc_Z = filtrage(Acc(:,3),'fir',30,20,Freq_ana); % Accélération verticale (obtenue grace à la PF)
    ...
    try
        ...
        %         Donnes.(acq).primResultats.minAPAy_AP(2) = mean(Donnes.(acq).CP_AP(1:round(evts(1)*Freq_ana))) - val_min;
        Donnes.(acq).primResultats.minAPAy_AP(2) = Donnes.(acq).CP_AP(round(evts(1)*Freq_ana)) - val_min; %modif XD
    catch Err_APA_AP
    ...
    try
        [Donnes.(acq).primResultats.APAy_ML(1) Donnes.(acq).primResultats.APAy_ML(2)] ...
                                               = trouve_APAy(Donnes.(acq).CP_ML(1:round(evts(1)*Freq_ana)));
    catch Err_APAy
        Donnes.(acq).primResultats.APAy_ML(1:2)= NaN;
    end
    
    try
        Donnes.(acq).primResultats.Largeur_pas = range(CP_filt(round(evts(3)*Freq_ana):round(evts(4)*Freq_ana),1)); 
        Donnes.(acq).primResultats.Longueur_pas = range(CP_filt(round(evts(3)*Freq_ana):round(evts(4)*Freq_ana),2));
    catch ERr_Step
