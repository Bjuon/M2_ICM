...
% Donnees.()   = structure contenant par (champ/acquisitions) les données d'intérêts (PF+VICON)
...
%             fin = length(t);
            ...
            Freq_ana = 500; %Fréquence d'acquisition NOtocord
        otherwise
    ...
    if isfield(DATA,'Analogs')
        ...
        if isnan(signal)
            signal = extrait_FSW_analog(DATA.Analogs); %% Le trigger est sur un canal nommé 'FSW'
        end
    ...
    try 
        ...
        if ind_1>10 % Cas ou l'acqisition commence tardivement avec l'initiation du pas
            Donnees.(acq).tMarkers.T0 = calcul_APA_T0_v4(CP_filt(1:ind_1,:),t(1:ind_1)) + t(1); % 1er evt biomécanique
        else
        ...
        if length(evts)<5
            ...
            Donnees.(acq).tMarkers.T0 = evts(1) + t(1); % 1er evt biomécanique
...
%     pause(0.5);
    ...
    if ~isempty(evts_fog) % Evts de Freezing
        Donnees.(acq).tMarkers.FOG = evts_fog; % Stockés sous forme [Start Stop Start Stop...]
    end
    
    if ~isempty(evts_dt) % Evts du Demi-Tour
        Donnees.(acq).tMarkers.DT = evts_dt; % Stockés sous forme [Start Stop ...] ??
    end 
    
    Donnees.(acq).t = t; %Temps
    Donnees.(acq).Fech = Freq_ana; %Fréquence d'échantillonage (Analogique)
    Donnees.(acq).Fech_vid = Freq_vid; %Fréquence d'échantillonage (Analogique)
    Donnees.(acq).CP_AP = CP_filt(:,2); %Déplacement AP du CP
    Donnees.(acq).CP_ML = CP_filt(:,1); %Déplacement ML du CP
    
    Donnees.(acq).V_CG_AP = V_CG_PF(:,2); % Vitesse du CG AP (intégration)
    Donnees.(acq).V_CG_ML = V_CG_PF(:,1); % Vitesse du CG ML (intégration)
    Donnees.(acq).V_CG_Z = V_CG_PF(:,3); % Vitesse verticale du CG (intégration)
    
    if ~isempty(V_CG_Der)
        Donnees.(acq).V_CG_AP_d = V_CG_Der(:,2); % Vitesse du CG (OBassin) AP (dérivation)
        Donnees.(acq).V_CG_Z_d = V_CG_Der(:,3); % Vitesse verticale du CG (OBassin) (dérivation)
    end 
    
    if ~isempty(V_CG_Vic)
        Donnees.(acq).V_CG_Vic_AP_d = V_CG_Vic(:,2); % Vitesse du CG de VICON AP (dérivation)
        Donnees.(acq).V_CG_Vic_Z_d = V_CG_Vic(:,3); % Vitesse verticale du CG de VICON (dérivation)
    end 
    
    Donnees.(acq).Acc_Z = filtrage(Acc(:,3),'fir',30,20,Freq_ana); % Accélération verticale (obtenue grace à la PF)
    ...
    % EMGs
    if isfield(DATA,'EMG')
        EMG.(acq).nom = DATA.EMG.nom; %On stocke les 4 premières entrées (par défaut) %%% Modifier si +ieurs voies à visualiser
