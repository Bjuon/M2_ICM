function [Moy tmp Std] = regroupe_acquisitions_v4(GN,C)
%% Calcul du corridor moyen et des paramètres moyens d'un ensembles d'acquisitions stockées dans la structure Sujet
%Entrés: structure GN contenant les acquisitions normalisées, C : structure contenant le côté (pour séparer les essais Gauche et Droite)
...
for j = 1:length(datas)
    d=1;
    g=1;
    if ~strcmp(datas{j},'Angles')
        for i = 1:length(acquisitions)
            if isfield(GN.(acquisitions{i}),datas{j})
...
%                             tmp_sub.(datas{j}).(sub_datas{k})(i,:) = GN.(acquisitions{i}).(datas{j}).(sub_datas{k})(1) - T0; %% (1) pour contourner les evenments du type FOG
                            tmp_sub.(datas{j}).(sub_datas{k})(i,:) = GN.(acquisitions{i}).(datas{j}).(sub_datas{k})(1)
                        else
                            try
