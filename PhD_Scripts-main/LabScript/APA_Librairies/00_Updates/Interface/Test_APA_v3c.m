function varargout = Test_APA_v3c(varargin)
% TEST_APA_V3C MATLAB code for Test_APA_v3c.fig
%      TEST_APA_V3C, by itself, creates a new TEST_APA_V3C or raises the existing
%      singleton*.
%
%      H = TEST_APA_V3C returns the handle to a new TEST_APA_V3C or the handle to
%      the existing singleton*.
%
%      TEST_APA_V3C('CALLBACK',hObject,eventData,handles,...) calls the local
%      function named CALLBACK in TEST_APA_V3C.M with the given input arguments.
%
%      TEST_APA_V3C('Property','Value',...) creates a new TEST_APA_V2 or raises the
%      existing singleton*.  Starting from the left, property value pairs are
%      applied to the GUI before Test_APA_v3c_OpeningFcn gets called.  An
%      unrecognized property name or invalid value makes property application
%      stop.  All inputs are passed to Test_APA_v3c_OpeningFcn via varargin.
%
...
% Edit the above text to modify the response to help Test_APA_v3c

% Last Modified by GUIDE v2.5 18-Feb-2014 16:33:16

% Begin initialization code - DO NOT EDIT
gui_Singleton = 1;
gui_State = struct('gui_Name',       mfilename, ...
    'gui_Singleton',  gui_Singleton, ...
    'gui_OpeningFcn', @Test_APA_v3c_OpeningFcn, ...
    'gui_OutputFcn',  @Test_APA_v3c_OutputFcn, ...
    'gui_LayoutFcn',  [] , ...
    'gui_Callback',   []);
...
%% Test_APA_v3c_OpeningFcn -Funcion principale
% --- Executes just before Test_APA_v3c is made visible.
function Test_APA_v3c_OpeningFcn(hObject, eventdata, handles, varargin)
...
% varargin   command line arguments to Test_APA_v3c (see VARARGIN)

% Choose default command line output for Test_APA_v3c
handles.output = hObject;

% Update handles structure
guidata(hObject, handles);

% UIWAIT makes Test_APA_v3c wait for user response (see UIRESUME)
% uiwait(handles.Test_APA_v3c);
set(gcf,'Name','Calcul des APA v.3c');
...
function varargout = Test_APA_v3c_OutputFcn(hObject, eventdata, handles)
...
try
    ...
    [Sujet EMG] = pretraitement_dataAPA_v7(files,dossier(1:end-1),button_cut);
...
try
    ...
    %Extraction des données d'intérêts
    button_cut = questdlg('Lire toute l''acquisition?','Durée acquisition','Oui','PF','PF');
    [Sujet EMG] = pretraitement_dataAPA_v7(files,dossier,button_cut);
...
try
    ...
    %Extraction des données d'intérêts
    button_cut = questdlg('Lire toute l''acquisition?','Durée acquisition','Oui','PF','PF');
    [Add Add_emg]= pretraitement_dataAPA_v7(files,dossier(1:end-1),button_cut);
...
%% Test_APA_v3c_ResizeFcn
% --- Executes when Test_APA_v3c is resized.
function Test_APA_v3c_ResizeFcn(hObject, eventdata, handles)
% hObject    handle to Test_APA_v3c (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
...
try
    ...
    %      hh4 = plot(axes4,tt,V_CG_Z,'r','Linewidth',1.5);
