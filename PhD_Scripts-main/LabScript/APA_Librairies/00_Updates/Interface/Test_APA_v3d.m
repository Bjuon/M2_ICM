function varargout = Test_APA_v3d(varargin)
% TEST_APA_V3D MATLAB code for Test_APA_v3d.fig
%      TEST_APA_V3D, by itself, creates a new TEST_APA_V3D or raises the existing
%      singleton*.
%
%      H = TEST_APA_V3D returns the handle to a new TEST_APA_V3D or the handle to
%      the existing singleton*.
%
%      TEST_APA_V3D('CALLBACK',hObject,eventData,handles,...) calls the local
%      function named CALLBACK in TEST_APA_V3D.M with the given input arguments.
%
%      TEST_APA_V3D('Property','Value',...) creates a new TEST_APA_V2 or raises the
%      existing singleton*.  Starting from the left, property value pairs are
%      applied to the GUI before Test_APA_v3d_OpeningFcn gets called.  An
%      unrecognized property name or invalid value makes property application
%      stop.  All inputs are passed to Test_APA_v3d_OpeningFcn via varargin.
%
...
% Edit the above text to modify the response to help Test_APA_v3d

% Last Modified by GUIDE v2.5 15-Apr-2014 17:46:24

% Begin initialization code - DO NOT EDIT
gui_Singleton = 1;
gui_State = struct('gui_Name',       mfilename, ...
    'gui_Singleton',  gui_Singleton, ...
    'gui_OpeningFcn', @Test_APA_v3d_OpeningFcn, ...
    'gui_OutputFcn',  @Test_APA_v3d_OutputFcn, ...
    'gui_LayoutFcn',  [] , ...
    'gui_Callback',   []);
...
%% Test_APA_v3d_OpeningFcn -Funcion principale
% --- Executes just before Test_APA_v3d is made visible.
function Test_APA_v3d_OpeningFcn(hObject, eventdata, handles, varargin)
...
% varargin   command line arguments to Test_APA_v3d (see VARARGIN)

% Choose default command line output for Test_APA_v3d
handles.output = hObject;

% Update handles structure
guidata(hObject, handles);

% UIWAIT makes Test_APA_v3d wait for user response (see UIRESUME)
% uiwait(handles.Test_APA_v3d);
set(gcf,'Name','Calcul des APA v.3d');
...
function varargout = Test_APA_v3d_OutputFcn(hObject, eventdata, handles)
...
try
    ...
    [Sujet EMG] = pretraitement_dataAPA_v7a(files,dossier(1:end-1),button_cut);
...
% --- Executes when Test_APA_v3d is resized.
function Test_APA_v3d_ResizeFcn(hObject, eventdata, handles)
% hObject    handle to Test_APA_v3d (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
