nom_fichier_res = 'Resultats_EMG_Intra_NonParam.xls';
STims = {'CER' 'SMA' 'SHAM'};
...
for f = 1:nb_files
    if f==1 && ischar(nom_fichiers)
        Sheet = extract_spaces(nom_fichiers(1:end-4)); % On nomme chaque feuille par sujet
    ...
    Nb_muscles = length(BdD); %% Une feuille par muscle
    c=0;
    Headers={};
    Moys_Pre=[];
    Std_Pre=[];
...
%             Ps(c,:) = batch_tTest(Pre,Post);
            Acq = Txt{stops(k+1)-1,1};
            tags = extract_tags(Acq);
            if ~isempty(str2num(tags{end}(end)))
                tags{end} = tags{end}(1:end-1);
            end
            Acq = tags{3};
            for ll=4:length(tags)
                Acq = [Acq '_' tags{ll}];
            end
            Condition{c} = Acq;
...
%             
    ...
    Params = BdD(i).noms;
    Ligne1 = {'Muscle (p-values)' 'TMS'};
    Ligne1 = [Ligne1 Params];
    vides = repmat({' '},1,length(Ligne1));
    Ligne2 = ['Muscle (MoyennesPre)' 'TMS' Params];   
    Ligne3 = ['Muscle (STDPre)' 'TMS' Params];
    Ligne4 = ['Muscle (MoyennesPost)' 'TMS' Params];   
    Ligne5 = ['Muscle (STDPost)' 'TMS' Params];
    kki = 1;
    for kk=1:size(N_stim_cumul,1)
        for kkk = 1:N_stim_cumul(kk,f)
            if kkk==1
                Row1{kki,1} = cell2mat(Muscle(kk));
            else
                Row1{kki,1} =[];
            end
            TMS{kki,1} = cell2mat(Condition(kkk));
            kki = kki + 1;
        end
    end
...
%     xlswrite(nom_fichier_res,Excl_p,[Sheet '_Wilcox']);
    xlswrite(nom_fichier_res,Write2,[Sheet '_Wilcox']);
%     xlswrite('P_values_old2.xls',Write2,[Sheet 'All_P_Wilcoxon']);
end
