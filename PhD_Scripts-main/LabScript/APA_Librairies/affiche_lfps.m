...
try
    ...
    for i=1:add
        ...
        for c = 1:length(contacts)
            Data_PE = change_point_analysis_cusum(current_PE.(contacts{c})');
    ...
    for c = 1:N_contact
        ...
        for s = 1:size(signals_to_process_pre,1)
            if ~chronux
                ...
                [S_base(:,:,c),F,T,P_base(:,:,s,c)] = spectrogram(signals_to_process_pre(s,:),window_width,overlap_width,vect_F,Fs);
            else
                [P_base(:,:,,s,c),F,T] = mtspecgramc(signals_to_process_pre(s,:),movingwin,parms);
            end
...
%             L=length(signals_to_process(s,:));
            ...
            setp_f = (Fmax-Fmin)/NFFT;
            vect_F = [Fmin:setp_f:Fmax];            
            [S(:,:,c),F,T,P(:,:,s,c)] = spectrogram(signals_to_process(s,:),window_width,overlap_width,vect_F,Fs);
            % F: vecteur des fréquences, T : vecteur temps
            % P: matrice des puissance (dim1: par fréquence; dim2: par temps; dim3: par essai; dim4: par contact)
            if normalisation
