function lfp  = affiche_lfps_beta_v4(listes_acqs)
...
contacts_Moy = {'ContraL01' 'ContraL12' 'ContraL23' 'IpsiL01' 'IpsiL12' 'IpsiL23'};
...
try
    ...
    for c=1:N_lfp
        ...
        PE.(save_PE).(contacts_Moy{c}) = Data_PE;
        PE_base.(save_PE).(contacts_Moy{c}) = Data_PE_base;
    end
...
%% Calcul d'un PE (en % de cycle d'initation de préparation): i.e. entre GO et T0
...
%                 current_lfp = lfp(ind_T0:end); % Après T0
                if ~isnan(FC2)
                    ...
                    current_lfp = [lfp_pre lfp]; % De -2 jusk'a la fin
                end
...
%                     [P_T0(i_check,:,c) F_T0] = psd(current_lfp(1:ind_T0),Fs,Fs, hanning(Fs), Fs/2, 'mean'); % Entre le GO et T0
%                     [P_base(i_check,:,c) F_base] = psd(lfp_pre,Fs,Fs, hanning(Fs), Fs/2, 'mean'); % Avant le GO
                catch ERr
...
%                     [Pp_base Ff_base] = psd(lfp_pre,Fs,Fs, hanning(Fs), Fs/2, 'mean'); % Avant T0
...
%         area(F_plot,nanmean(P_plot(:,:,c),1),'Linewidth',1.5,'FaceColor','b'); %alpha 0.85
        plot_vector(nanmean(P_plot(:,:,c),1),F_plot,'l',[],'b');
        if comparaison
            hold on
            if ~baseline
%                 area(CI.F,nanmean(CI.P(:,:,c),1),'Linewidth',1.25,'FaceColor','r');
                plot_vector(nanmean(CI.P(:,:,c),1),CI.F,'l',[],'r');
            else
%                 area(CI.F_base,nanmean(CI.P_base(:,:,c),1),'Linewidth',1.25,'FaceColor','r');
                plot_vector(nanmean(CI.P_base(:,:,c),1),CI.F_base,'l',[],'r');
            end
        end
%         if c<4
%             set(gca,'ylim',[0 max(max(nanmean(P_plot(:,:,1:3),1)))/1.5]); %alpha 0.85 % mean(P_T0(:,1:80,c))/2
%         else
%             set(gca,'ylim',[0 max(max(nanmean(P_plot(:,:,4:6),1)))/1.5]); %alpha 0.85
%         end
        set(gca,'xlim',[0 49]);
        grid
        title(contacts_Moy{c});
...
%         area(F_plot,nanmean(P_plot(:,:,c),1),'Linewidth',2,'FaceColor','b'); %alpha 0.85
        plot_vector(nanmean(P_plot(:,:,c),1),F_plot,'l',[],'b');
        if comparaison
            hold on
            if ~baseline
...
%         if c<4
%             set(gca,'ylim',[0 max(max(nanmean(P_plot(:,round(length(F_plot))/2:end,1:3),1)))]); %alpha 0.85
%         else
%             set(gca,'ylim',[0 max(max(nanmean(P_plot(:,round(length(F_plot))/2:end,4:6),1)))]); %alpha 0.85
%         end
        ...
        title(contacts_Moy{c});
%         ylabel('DSP Moyenne (V²/Hz)'); xlabel('Frequence Hz');
%         ylabel('log |dB|'); xlabel('Frequence Hz');
...
%         tags = extract_tags(acquisitions{acqs(1)});
        ...
        print(f,'-djpeg','-r300',[path '\' file_to_print '_0-45Hz']);
        print(f1,'-djpeg','-r300',[path '\' file_to_print '_55-100Hz']);
