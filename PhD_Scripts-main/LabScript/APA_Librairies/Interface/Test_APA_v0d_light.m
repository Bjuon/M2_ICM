function varargout = Test_APA_v0d_light(varargin)
% TEST_APA_V0D_LIGHT MATLAB code for Test_APA_v0d_light.fig
%      TEST_APA_V0D_LIGHT, by itself, creates a new TEST_APA_V0D_LIGHT or raises the existing
%      singleton*.
%
%      H = TEST_APA_V0D_LIGHT returns the handle to a new TEST_APA_V0D_LIGHT or the handle to
%      the existing singleton*.
%
%      TEST_APA_V0D_LIGHT('CALLBACK',hObject,eventData,handles,...) calls the local
%      function named CALLBACK in TEST_APA_V0D_LIGHT.M with the given input arguments.
%
%      TEST_APA_V0D_LIGHT('Property','Value',...) creates a new TEST_APA_V0D_LIGHT or raises the
%      existing singleton*.  Starting from the left, property value pairs are
%      applied to the GUI before Test_APA_v0d_light_OpeningFcn gets called.  An
%      unrecognized property name or invalid value makes property application
%      stop.  All inputs are passed to Test_APA_v0d_light_OpeningFcn via varargin.
%
...
% Edit the above text to modify the response to help Test_APA_v0d_light

% Last Modified by GUIDE v2.5 05-Apr-2012 19:34:47

% Begin initialization code - DO NOT EDIT
gui_Singleton = 1;
gui_State = struct('gui_Name',       mfilename, ...
                   'gui_Singleton',  gui_Singleton, ...
                   'gui_OpeningFcn', @Test_APA_v0d_light_OpeningFcn, ...
                   'gui_OutputFcn',  @Test_APA_v0d_light_OutputFcn, ...
                   'gui_LayoutFcn',  [] , ...
                   'gui_Callback',   []);
...
% --- Executes just before Test_APA_v0d_light is made visible.
function Test_APA_v0d_light_OpeningFcn(hObject, ~, handles, varargin)
...
% varargin   command line arguments to Test_APA_v0d_light (see VARARGIN)

% Choose default command line output for Test_APA_v0d_light
handles.output = hObject;

% Update handles structure
guidata(hObject, handles);

% UIWAIT makes Test_APA_v0d_light wait for user response (see UIRESUME)
...
set(gcf,'Name','Calcul des APA v.0d');
...
function varargout = Test_APA_v0d_light_OutputFcn(hObject, eventdata, handles) 
...
Sujet.(acq_courante).primResultats.VyFO1 = [ind Vy_FO1];
...
global haxes3 Sujet h_marks_VyFO1
...
if get(findobj('tag','V_der'),'Value')
    ...
    VyFO1 = Sujet.(acq_courante).V_CG_AP_d(ind);
    h_marks_VyFO1 = plot(haxes3,Sujet.(acq_courante).t(ind),VyFO1,'x','Markersize',11);
    %Réactualisation de VyFO1 et recalcul des largeur/longueur du pas
    Sujet.(acq_courante).primResultats.VyFO1 = [ind VyFO1];
else
    warndlg('VyFO1 dépend de TO!!');
end
...
if get(findobj('tag','V_der'),'Value')
    ...
    ind = floor(Manual_click(1)*Sujet.(acq_courante).Fech);
    efface_marqueur_test(h_marks_V2);
    V2 = Sujet.(acq_courante).V_CG_AP_d(ind);
    h_marks_V2 = plot(haxes3,Sujet.(acq_courante).t(ind),V2,'x','Markersize',11);
    %Réactualisation de VyFO1 et recalcul des largeur/longueur du pas
    Sujet.(acq_courante).primResultats.V2 = [ind V2];
else
    warndlg('V2 dépend de FC1!!');
end
