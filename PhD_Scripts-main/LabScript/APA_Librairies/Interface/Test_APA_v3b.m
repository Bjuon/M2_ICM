function varargout = Test_APA_v3b(varargin)
% TEST_APA_V3B MATLAB code for Test_APA_v3b.fig
%      TEST_APA_V3B, by itself, creates a new TEST_APA_V3B or raises the existing
%      singleton*.
%
%      H = TEST_APA_V3B returns the handle to a new TEST_APA_V3B or the handle to
%      the existing singleton*.
%
%      TEST_APA_V3B('CALLBACK',hObject,eventData,handles,...) calls the local
%      function named CALLBACK in TEST_APA_V3B.M with the given input arguments.
%
%      TEST_APA_V3B('Property','Value',...) creates a new TEST_APA_V2 or raises the
%      existing singleton*.  Starting from the left, property value pairs are
%      applied to the GUI before Test_APA_v3b_OpeningFcn gets called.  An
%      unrecognized property name or invalid value makes property application
%      stop.  All inputs are passed to Test_APA_v3b_OpeningFcn via varargin.
%
...
% Edit the above text to modify the response to help Test_APA_v3b

% Last Modified by GUIDE v2.5 18-Feb-2014 15:54:18

% Begin initialization code - DO NOT EDIT
gui_Singleton = 1;
gui_State = struct('gui_Name',       mfilename, ...
                   'gui_Singleton',  gui_Singleton, ...
                   'gui_OpeningFcn', @Test_APA_v3b_OpeningFcn, ...
                   'gui_OutputFcn',  @Test_APA_v3b_OutputFcn, ...
                   'gui_LayoutFcn',  [] , ...
                   'gui_Callback',   []);
...
function Test_APA_v3b_OpeningFcn(hObject, eventdata, handles, varargin)
...
set(gcf,'Name','Calcul des APA v.3a');
...
function varargout = Test_APA_v3b_OutputFcn(hObject, eventdata, handles) 
...
try
    ...
    T0_vid = 100;
    FC2_vid = round(length(Sujet.(acq_courante).V_CG_AP_d)/2); % Au pif
end
...
[Sujet EMG] = pretraitement_dataAPA_v6(files,dossier(1:end-1),button_cut);
...
%Extraction des données d'intérêts
button_cut = questdlg('Lire toute l''acquisition?','Durée acquisition','Oui','PF','PF');
[Sujet EMG] = pretraitement_dataAPA_v6(files,dossier,button_cut);
...
%Extraction des données d'intérêts
button_cut = questdlg('Lire toute l''acquisition?','Durée acquisition','Oui','PF','PF');
[Add Add_emg]= pretraitement_dataAPA_v6(files,dossier(1:end-1),button_cut);
...
button_cut = questdlg('Lire toute l''acquisition?','Durée acquisition','Oui','PF','PF');
[Add Add_emg] = pretraitement_dataAPA_v6(files,dossier,button_cut);
...
% LFP = structure contenant les données LFP par essai (champ), reéchantillonnée à 200Hz (pou affichage synchrone avec autres données)
...
switch buttonExp
    ...
    case 'Analog/Kin'
        ...
        save_var_anlg = [dossier '\' Subject_data.ID '_Anlg_Dec_lena'];
        try
        ...
        try
            ...
            if ~isempty(h_multi_emg)
                ...
                save_var_emg = [dossier '\' Subject_data.ID '_EMG_Dec_lena'];
                eval(['save ' save_var_emg ' h b e']);
            end
            
            if ~isempty(h_multi_kin)
                h = h_multi_kin; b = b_multi_kin;
                save_var_kin = [dossier '\' Subject_data.ID '_Kinemat_Dec_lena'];
...
%     affiche_multidata(liste,PF,muscles,contacts);  % Affichage normale des signaux
    affiche_multidata_v3(liste,PF,muscles,contacts); % Pour afficher avec spectrograms (optionnel)
