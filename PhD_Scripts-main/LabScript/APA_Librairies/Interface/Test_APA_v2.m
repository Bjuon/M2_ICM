...
% Sujet = structure contenant les données PF+EMG et marqueurs temporels/Trigger par essai (champ)
...
% h_lena = structre d'entête pour l'export des LFP au format lena (si existe), créée lors du chargement du fichier d'enregistrement continu
% b_lena = matrice binaire contenant les valeurs des signaux LFP, créée lors du chargement du fichier d'enregistrement continu
...
% Création des structures pour export .lena
...
% Préconditionnement pour export .lena
try
...
%         liste_acqs_lfp = extract_sync_lfp(strcat(dossier_sync,file_sync));
    ...
    case 'Manuelle' % Pour chaque trigger ou choisit l'acquisition correspondante
        ...
        a=0;
        for l=1:N_trigs
            [acqs,v] = listdlg('PromptString',{'Liste des acquisitions chargés',['Acquisition - Trigger N° ' num2str(l)]},...
                'ListSize',[300 300],...
                'ListString',listes_acqs,...
...
%     ind_fin = ind_trigs(l) + floor(duree_essai*fs);
    ...
    for c=1:length(channels)
        ...
        try
            ...
            data_lena = col(c+1,ind_trigs(l)- 2*hdr.fs:ind_trigs(l) + 5*hdr.fs); %2 secondes avant l'instruction de marche jusqu'à 5 secondes après
        catch not_enough_pre_data
            data_base = NaN*ones(1,2*hdr.fs);
            data_base(end-ind_debut+1:end)= col_post(c+1,1:ind_debut);
            data_lena = [NaN*ones(1,2*hdr.fs - ind_trigs(l)+1) col(c+1,1:ind_trigs(l) + 5*hdr.fs)]; %2 secondes avant l'instruction de marche jusqu'à 5 secondes après
        end
        ...
        try
            ...
            LFP_base.(liste_acqs_lfp{l}).(['Contact' channels{c}]) = data_base;% (1:hdr.fs/Fech_pf:end); Non reechantillonnés
        catch ERR
...
if isempty(e_lena)
    warndlg('Structure des Evènements non créée/exportée!');
end
...
try
    ...
    disp('Pas d''export lena découpé');
end
