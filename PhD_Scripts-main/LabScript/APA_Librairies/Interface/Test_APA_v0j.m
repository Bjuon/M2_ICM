function varargout = Test_APA_v0j(varargin)
% TEST_APA_V0J MATLAB code for Test_APA_v0j.fig
%      TEST_APA_V0J, by itself, creates a new TEST_APA_V0J or raises the existing
%      singleton*.
%
%      H = TEST_APA_V0J returns the handle to a new TEST_APA_V0J or the handle to
%      the existing singleton*.
%
%      TEST_APA_V0J('CALLBACK',hObject,eventData,handles,...) calls the local
%      function named CALLBACK in TEST_APA_V0J.M with the given input arguments.
%
%      TEST_APA_V0J('Property','Value',...) creates a new TEST_APA_V0J or raises the
%      existing singleton*.  Starting from the left, property value pairs are
%      applied to the GUI before Test_APA_v0j_OpeningFcn gets called.  An
%      unrecognized property name or invalid value makes property application
%      stop.  All inputs are passed to Test_APA_v0j_OpeningFcn via varargin.
%
...
% Edit the above text to modify the response to help Test_APA_v0j

% Last Modified by GUIDE v2.5 15-May-2012 12:11:37

% Begin initialization code - DO NOT EDIT
gui_Singleton = 1;
gui_State = struct('gui_Name',       mfilename, ...
                   'gui_Singleton',  gui_Singleton, ...
                   'gui_OpeningFcn', @Test_APA_v0j_OpeningFcn, ...
                   'gui_OutputFcn',  @Test_APA_v0j_OutputFcn, ...
                   'gui_LayoutFcn',  [] , ...
                   'gui_Callback',   []);
...
% --- Executes just before Test_APA_v0j is made visible.
function Test_APA_v0j_OpeningFcn(hObject, eventdata, handles, varargin)
...
% varargin   command line arguments to Test_APA_v0j (see VARARGIN)

% Choose default command line output for Test_APA_v0j
handles.output = hObject;

% Update handles structure
guidata(hObject, handles);

% UIWAIT makes Test_APA_v0j wait for user response (see UIRESUME)
% uiwait(handles.figure1);
set(gcf,'Name','Calcul des APA v.0j');
...
function varargout = Test_APA_v0j_OutputFcn(hObject, eventdata, handles) 
...
if ~isempty(Corridors) && ~isempty(liste_actuelle)
    button = questdlg('Choix des acquisitions à charger sur la liste?','Mise à jour liste','Tous','Moyennes','Dernier','Dernier');
        switch button
            case 'Moyennes'
...
if ~Notocord
    ...
    Resultats.(acq_courante) = calculs_parametres_initiationPas_Not(Sujet.(acq_courante));
end
...
try
    ...
    T0 = round(Sujet_tmp.(acq_choisie).tMarkers.T0*Sujet_tmp.(acq_choisie).Fech);
     ...
     plot(axes1,Sujet_tmp.(acq_choisie).t(1:end-T0),Sujet_tmp.(acq_choisie).CP_AP(T0:end-1)); axis(axes1,'tight');
     plot(axes2,Sujet_tmp.(acq_choisie).t(1:end-T0),Sujet_tmp.(acq_choisie).CP_ML(T0:end-1)); axis(axes2,'tight');
     plot(axes3,Sujet_tmp.(acq_choisie).t(1:end-T0),Sujet_tmp.(acq_choisie).V_CG_AP(T0:end-1)); axis(axes3,'tight');
     plot(axes4,Sujet_tmp.(acq_choisie).t(1:end-T0),Sujet_tmp.(acq_choisie).V_CG_Z(T0:end-1)); axis(axes4,'tight');
catch ERR
...
global Sujet dossier EMG Resultats Notocord
...
for corr=1:length(i)
    Sujet = rmfield(Sujet,listes_corr{i(corr)});
    Corridors = rmfield(Corridors,listes_corr{i(corr)});
    Resultats = rmfield(Resultats,listes_corr{i(corr)});
end
