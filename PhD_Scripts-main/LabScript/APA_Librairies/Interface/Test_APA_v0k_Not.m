function varargout = Test_APA_v0k_Not(varargin)
% TEST_APA_V0K MATLAB code for Test_APA_v0k.fig
%      TEST_APA_V0K, by itself, creates a new TEST_APA_V0K or raises the existing
%      singleton*.
%
%      H = TEST_APA_V0K returns the handle to a new TEST_APA_V0K or the handle to
%      the existing singleton*.
%
%      TEST_APA_V0K('CALLBACK',hObject,eventData,handles,...) calls the local
%      function named CALLBACK in TEST_APA_V0K.M with the given input arguments.
%
%      TEST_APA_V0K('Property','Value',...) creates a new TEST_APA_V0K or raises the
%      existing singleton*.  Starting from the left, property value pairs are
%      applied to the GUI before Test_APA_v0k_OpeningFcn gets called.  An
%      unrecognized property name or invalid value makes property application
%      stop.  All inputs are passed to Test_APA_v0k_OpeningFcn via varargin.
%
...
% Edit the above text to modify the response to help Test_APA_v0k

% Last Modified by GUIDE v2.5 04-Jun-2012 14:12:05

% Begin initialization code - DO NOT EDIT
gui_Singleton = 1;
gui_State = struct('gui_Name',       mfilename, ...
                   'gui_Singleton',  gui_Singleton, ...
                   'gui_OpeningFcn', @Test_APA_v0k_OpeningFcn, ...
                   'gui_OutputFcn',  @Test_APA_v0k_OutputFcn, ...
                   'gui_LayoutFcn',  [] , ...
                   'gui_Callback',   []);
...
% --- Executes just before Test_APA_v0k is made visible.
function Test_APA_v0k_OpeningFcn(hObject, eventdata, handles, varargin)
...
% varargin   command line arguments to Test_APA_v0k (see VARARGIN)

% Choose default command line output for Test_APA_v0k
handles.output = hObject;

% Update handles structure
guidata(hObject, handles);

% UIWAIT makes Test_APA_v0k wait for user response (see UIRESUME)
% uiwait(handles.figure1);
set(gcf,'Name','Calcul des APA v.0k');
...
uimenu(h2,'Label','Ajouter acquistions .c3d','Callback',@ajouter_acquisitions) ;
uimenu(h2,'Label','Ajouter dossier .c3d','Callback',@ajouter_dossier) ;
...
uimenu(g,'Label','AJOUTER AU CORRIDOR','handlevisibility','on','Callback',@Corridors_add,'Tag','Corridors_add','Enable','off') ;
uimenu(g,'Label','AJOUTER AU GROUPE','handlevisibility','on','Callback',@Group_subjects_add,'Tag','Group_subjects_add','Enable','off') ;
uimenu(g,'Label','CHARGER GROUPE','handlevisibility','on','Callback',@Group_subjects_load,'Enable','off') ;

% NOTOCORD load MENU
n = uimenu('Parent',hObject,'Label','Notocord','Tag','menu_notocord','handlevisibility','On','Enable','on') ;
% uimenu(n,'Label','Charger acquisitions (*.xls)','Callback',@load_notocord_files); %%Créeer les fonctions plus tard
...
function varargout = Test_APA_v0k_OutputFcn(hObject, eventdata, handles) 
...
global Sujet EMG Resultats Corridors Group
...
%             set(findobj('tag','listbox1'),'String',fieldnames(Sujet));
        case 'Ajouter Corridors'
            if ~isempty(Add_corr)
                ...
                msgbox('Pas de corridors existants pour ajour!');
            end
        otherwise
            disp('Pas d''ajout');
    end
...
global Sujet Resultats Corridors Subject_data EMG Activation_EMG Activation_EMG_percycle Notocord
...
eval(['save ' Subject_data.ID ' Sujet Resultats Corridors Subject_data EMG liste_actuelle Activation_EMG Activation_EMG_percycle Notocord']);
...
global Sujet Resultats Corridors Activation_EMG_percycle Notocord
    ...
    try
        if isfield(listes_acqs{acqs(i)},Activation_EMG_percycle) % Moyenneage des activations EMG (si existent)
           % Rassembler les periodes d'activation EMG
            Group_emg.(listes_acqs{acqs(i)}) = Activation_EMG_percycle.(listes_acqs{acqs(i)});
        else
            disp(['Pas d''EMG pour l''acquisition: ' listes_acqs{acqs(i)}]);
        end
    catch ERR
...
EMG_moy = regroupe_EMGs(Group_emg);
...
if strcmp(button,'Oui') % On supprime uniquement de la liste les acquisitions qui ont été prises dans les groupe
    ...
    function Corridors_add(hObject, eventdata, handles)
        ...
        [Acq_moy_new Data_group_new Ecarts_acqs_new] = regroupe_acquisitions(Group_norm);
        ...
        [Acq_moy Data_group2 Ecarts_acqs] = regroupe_acquisitions(Acqs_moy);
        [Res_moy Res_group Ecarts_res] = regroupe_acquisitions(Moy_data);
...
global Sujet Resultats Corridors Activation_EMG_percycle Notocord Group
...
[Acq_moy Data_group2 Ecarts_acqs] = regroupe_acquisitions(Group_norm2);
[Res_moy Res_group Ecarts_res] = regroupe_acquisitions(Moy_data);
...
legendes={};
    ...
    for k=1:length(i)
        ...
        Offset_CPAP = Sujet_tmp.(listes_corr{i(k)}).CP_AP(1)*1e-2;
%         h_corr_CP_AP = stdshade(Corridors.(listes_corr{i(k)}).CP_AP,0.4,[0.5/k 0.5/k 1],Corridors.(listes_corr{i(k)}).t(1,:),1,axes1);
        h_corr_CP_AP = stdshade(Corridors.(listes_corr{i(k)}).CP_AP*1e-2-Offset_CPAP,0.3,[(k-0.5)/length(i) (k-1)/length(i) k/length(i)],t,1,axes1);
    ...
    for k=1:length(i)
        ...
        Offset_CPML = Sujet_tmp.(listes_corr{i(k)}).CP_ML(1)*1e-2;
        h_corr_CP_ML = stdshade(Corridors.(listes_corr{i(k)}).CP_ML*1e-2-Offset_CPML,0.3,[(k-0.5)/length(i) (k-1)/length(i) k/length(i)],t,1,axes2);
    end
...
try
    ...
    T0 = round(Sujet_tmp.(acq_choisie).tMarkers.T0*Sujet_tmp.(acq_choisie).Fech);
    
    dimm = length(t);
    FC1 = round(Sujet_tmp.(acq_choisie).tMarkers.FC1*Sujet_tmp.(acq_choisie).Fech) - T0;
    FC1_corr = round(T_FC1_base*Sujet_tmp.(listes_corr{i(1)}).Fech);
    decalage_V = (FC1 - FC1_corr);
...
%     V_CG_Z_d = Sujet_tmp.(acq_choisie).V_CG_Z_d;
    ...
    txt(regexp(acq_choisie,'_')) = ' ';
     ...
     Offset_CPAP = Sujet_tmp.(acq_choisie).CP_AP(1)-Sujet_tmp.(listes_corr{i(1)}).CP_AP(1);
     hh1 = plot(axes1,t,Sujet_tmp.(acq_choisie).CP_AP(T0:dimm+T0-1)-Offset_CPAP,'r','Linewidth',1.5);  
     set(hh1,'Displayname',txt);
     affiche_label(hh1,txt,axes1);    axis(axes1,'tight');
     
     Offset_CPML = Sujet_tmp.(acq_choisie).CP_ML(1)-Sujet_tmp.(listes_corr{i(1)}).CP_ML(1);
     hh2 = plot(axes2,t,Sujet_tmp.(acq_choisie).CP_ML(T0:dimm+T0-1)-Offset_CPML,'r','Linewidth',1.5); 
