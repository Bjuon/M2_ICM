function varargout = Test_APA_v0b_light(varargin)
% TEST_APA_V0B_LIGHT MATLAB code for Test_APA_v0b_light.fig
%      TEST_APA_V0B_LIGHT, by itself, creates a new TEST_APA_V0B_LIGHT or raises the existing
%      singleton*.
%
%      H = TEST_APA_V0B_LIGHT returns the handle to a new TEST_APA_V0B_LIGHT or the handle to
%      the existing singleton*.
%
%      TEST_APA_V0B_LIGHT('CALLBACK',hObject,eventData,handles,...) calls the local
%      function named CALLBACK in TEST_APA_V0B_LIGHT.M with the given input arguments.
%
%      TEST_APA_V0B_LIGHT('Property','Value',...) creates a new TEST_APA_V0B_LIGHT or raises the
%      existing singleton*.  Starting from the left, property value pairs are
%      applied to the GUI before Test_APA_v0b_light_OpeningFcn gets called.  An
%      unrecognized property name or invalid value makes property application
%      stop.  All inputs are passed to Test_APA_v0b_light_OpeningFcn via varargin.
%
...
% Edit the above text to modify the response to help Test_APA_v0b_light

% Last Modified by GUIDE v2.5 30-Mar-2012 09:48:04

% Begin initialization code - DO NOT EDIT
gui_Singleton = 1;
gui_State = struct('gui_Name',       mfilename, ...
                   'gui_Singleton',  gui_Singleton, ...
                   'gui_OpeningFcn', @Test_APA_v0b_light_OpeningFcn, ...
                   'gui_OutputFcn',  @Test_APA_v0b_light_OutputFcn, ...
                   'gui_LayoutFcn',  [] , ...
                   'gui_Callback',   []);
...
% --- Executes just before Test_APA_v0b_light is made visible.
function Test_APA_v0b_light_OpeningFcn(hObject, ~, handles, varargin)
...
% varargin   command line arguments to Test_APA_v0b_light (see VARARGIN)

% Choose default command line output for Test_APA_v0b_light
handles.output = hObject;

% Update handles structure
guidata(hObject, handles);

% UIWAIT makes Test_APA_v0b_light wait for user response (see UIRESUME)
% uiwait(handles.figure1);

scrsz = get(0,'ScreenSize');
set(hObject,'Position',[scrsz(3)/30 scrsz(4)/30 scrsz(3)*9/20 scrsz(4)*9/20]);
...
set(gcf,'Name','Calcul des APA v.0b_light');
...
function varargout = Test_APA_v0b_light_OutputFcn(hObject, eventdata, handles) 
...
global haxes1 haxes2 haxes3 haxes4 Sujet acq_courante h_marks_T0 h_marks_HO h_marks_TO h_marks_FC1 h_marks_FO2 h_marks_FC2
...
%Activation des fenêtres et legendes d'axes 
...
global haxes1 haxes2 haxes3 haxes4 Sujet dossier Multiplot
...
% Mise à jour listbox1
set(findobj('tag','listbox1'),'Visible','On');
set(findobj('tag','listbox1'), 'String',fieldnames(Sujet));

% Activation interface (si Off)
set(findobj('tag','togglebutton1'),'Visible','On');
set(findobj('tag','AutoScale'),'Visible','On');
set(findobj('tag','Multiplot'),'Visible','On');
...
Sujet.(acq_courante).primResultats.minAPAy_AP = [ind Sujet.(acq_courante).CP_AP(ind)];
...
%Stockage du résultats
Sujet.(acq_courante).primResultats.APAy_ML = [ind abs(mean(Sujet.(acq_courante).CP_ML(1:15)) - Extrema)];
...
global Sujet acq_courante Resultats_courant
...
if strcmp(button,'Oui')
    ...
    ecrireQR_xls(Resultats_courant,[fichier '.xls'],sheet);
else
    disp('Attention données non exportées!');
end
...
global Sujet Resultats
inputdlg('Entrez le nom du groupe','Moyennage');
