function varargout = Test_APA_v0b(varargin)
% TEST_APA_V0B MATLAB code for Test_APA_v0b.fig
%      TEST_APA_V0B, by itself, creates a new TEST_APA_V0B or raises the existing
%      singleton*.
%
%      H = TEST_APA_V0B returns the handle to a new TEST_APA_V0B or the handle to
%      the existing singleton*.
%
%      TEST_APA_V0B('CALLBACK',hObject,eventData,handles,...) calls the local
%      function named CALLBACK in TEST_APA_V0B.M with the given input arguments.
%
%      TEST_APA_V0B('Property','Value',...) creates a new TEST_APA_V0B or raises the
%      existing singleton*.  Starting from the left, property value pairs are
%      applied to the GUI before Test_APA_v0b_OpeningFcn gets called.  An
%      unrecognized property name or invalid value makes property application
%      stop.  All inputs are passed to Test_APA_v0b_OpeningFcn via varargin.
%
...
% Edit the above text to modify the response to help Test_APA_v0b

% Last Modified by GUIDE v2.5 02-Apr-2012 16:38:24

% Begin initialization code - DO NOT EDIT
gui_Singleton = 1;
gui_State = struct('gui_Name',       mfilename, ...
                   'gui_Singleton',  gui_Singleton, ...
                   'gui_OpeningFcn', @Test_APA_v0b_OpeningFcn, ...
                   'gui_OutputFcn',  @Test_APA_v0b_OutputFcn, ...
                   'gui_LayoutFcn',  [] , ...
                   'gui_Callback',   []);
...
% --- Executes just before Test_APA_v0b is made visible.
function Test_APA_v0b_OpeningFcn(hObject, ~, handles, varargin)
...
% varargin   command line arguments to Test_APA_v0b (see VARARGIN)

% Choose default command line output for Test_APA_v0b
handles.output = hObject;

% Update handles structure
guidata(hObject, handles);

% UIWAIT makes Test_APA_v0b wait for user response (see UIRESUME)
...
set(gcf,'Name','Calcul des APA v.0b');
...
function varargout = Test_APA_v0b_OutputFcn(hObject, eventdata, handles) 
...
switch flag_afficheV
    ...
    case 1 
        if flags_V(2) %Courbes dérivées
            plot(haxes3,Sujet.(acq_courante).t,Sujet.(acq_courante).V_CG_AP_d);
            plot(haxes4,Sujet.(acq_courante).t,Sujet.(acq_courante).V_CG_Z_d);
    ...
    case 2 %Les 2
        plot(haxes3,Sujet.(acq_courante).t,Sujet.(acq_courante).V_CG_AP); hold on
        plot(haxes3,Sujet.(acq_courante).t,Sujet.(acq_courante).V_CG_AP_d,'r-');
        plot(haxes4,Sujet.(acq_courante).t,Sujet.(acq_courante).V_CG_Z); hold on
