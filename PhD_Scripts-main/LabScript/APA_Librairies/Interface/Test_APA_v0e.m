function varargout = Test_APA_v0e(varargin)
% TEST_APA_V0E MATLAB code for Test_APA_v0e.fig
%      TEST_APA_V0E, by itself, creates a new TEST_APA_V0E or raises the existing
%      singleton*.
%
%      H = TEST_APA_V0E returns the handle to a new TEST_APA_V0E or the handle to
%      the existing singleton*.
%
%      TEST_APA_V0E('CALLBACK',hObject,eventData,handles,...) calls the local
%      function named CALLBACK in TEST_APA_V0E.M with the given input arguments.
%
%      TEST_APA_V0E('Property','Value',...) creates a new TEST_APA_V0E or raises the
%      existing singleton*.  Starting from the left, property value pairs are
%      applied to the GUI before Test_APA_v0e_OpeningFcn gets called.  An
%      unrecognized property name or invalid value makes property application
%      stop.  All inputs are passed to Test_APA_v0e_OpeningFcn via varargin.
%
...
% Edit the above text to modify the response to help Test_APA_v0e

% Last Modified by GUIDE v2.5 11-Apr-2012 14:41:21

% Begin initialization code - DO NOT EDIT
gui_Singleton = 1;
gui_State = struct('gui_Name',       mfilename, ...
                   'gui_Singleton',  gui_Singleton, ...
                   'gui_OpeningFcn', @Test_APA_v0e_OpeningFcn, ...
                   'gui_OutputFcn',  @Test_APA_v0e_OutputFcn, ...
                   'gui_LayoutFcn',  [] , ...
                   'gui_Callback',   []);
...
% --- Executes just before Test_APA_v0e is made visible.
function Test_APA_v0e_OpeningFcn(hObject, eventdata, handles, varargin)
...
% varargin   command line arguments to Test_APA_v0e (see VARARGIN)

% Choose default command line output for Test_APA_v0e
handles.output = hObject;

% Update handles structure
guidata(hObject, handles);

% UIWAIT makes Test_APA_v0e wait for user response (see UIRESUME)
...
uimenu(h1,'Label','Charger acqusitions','Callback',@uipushtool1_ClickedCallback) ; %% uipushtool1_ClickedCallback(findobj('tag','uipushtool1'), eventdata, handles))
...
% uimenu(h1,'Label','GROUPE','Callback','file(''choix_groupe'');','handlevisibility','off') ;
uimenu(h,'Label','DONNEES PATIENT','Callback','file(''patient_data'');','handlevisibility','off') ;

% --- Outputs from this function are returned to the command line.
function varargout = Test_APA_v0e_OutputFcn(hObject, eventdata, handles) 
...
eval(['save ' var ' Sujet Resultats Corridors']);
...
global Sujet Resultats clean
% hObject    handle to Clean_data (see GCBO)

%Choix de la courbe/variable de control (entre déplacements CP ou vitesses CG)
...
for i=1:length(acqs)
    ...
    if ~isfield(listes_acqs{acqs(i)},Resultats)
...
% else %Affichage dans une nouvelle fenêtre
%     figure
%     subplot(411);
%     stdshade(Data_group.CP_AP,0.4,'b',Data_group.t(1,:),1);
%     subplot(412);
%     stdshade(Data_group.CP_ML,0.4,'b',Data_group.t(1,:),1);
%     subplot(413);
%     stdshade(Data_group.V_CG_AP,0.4,'b',Data_group.t(1,:),1);
%     subplot(414);
%     stdshade(Data_group.V_CG_ML,0.4,'b',Data_group.t(1,:),1);
end

catch ERR
    disp('Arret création groupe');
...
global axes1 axes2 axes3 axes4 Corridors Sujet list
...
global axes1 axes2 axes3 axes4 Sujet list
        
%Récupération de l'acquisition séléctionnée
contents = cellstr(get(list,'String'));
acq_choisie = contents{get(list,'Value')};

 plot(axes1,Sujet.(acq_choisie).t,Sujet.(acq_choisie).CP_AP);
 plot(axes2,Sujet.(acq_choisie).t,Sujet.(acq_choisie).CP_ML);
 plot(axes3,Sujet.(acq_choisie).t,Sujet.(acq_choisie).V_CG_AP);
 plot(axes4,Sujet.(acq_choisie).t,Sujet.(acq_choisie).V_CG_Z);   
