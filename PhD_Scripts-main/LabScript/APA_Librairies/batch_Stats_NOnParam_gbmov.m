nom_fichier_res = 'Resultats_Inter_NonParam_GBMOV.xls';
compare = {'ONvsOFF' 'ONvsOFF' 'SvsR' 'SvsR'};
...
for f = 1:nb_files
    if f==1 && ischar(nom_fichiers)
        Sheet = extract_spaces(nom_fichiers(1:end-5)); % On nomme chaque acquisition en commencant par le dossier ou elle se trouve
        fichier = nom_fichiers;
    else
        Sheet = [extract_spaces(nom_fichiers{f}(1:end-5))];
...
%             
    ...
    Params = BdD(i).noms;
    Ligne1 = {'P-values' 'Condition'};
    Ligne1 = [Ligne1 Params];
    vides = repmat({' '},1,length(Ligne1));
    Ligne2 = ['MoyennesON' 'Condition' Params];   
    Ligne3 = ['STD_ON' 'Condition' Params];
    Ligne4 = ['MoyennesOFF' 'Condition' Params];   
    Ligne5 = ['STDPost_OFF)' 'Condition' Params];
