...
if isempty(varargin)  % Check if the function is called without additional arguments
    clear;             
    argin = false;        % Set flag to indicate no arguments were provided
    tStart = tic;         % Start a timer to measure execution time
else 
    clearvars -except varargin  % Clear all variables except varargin to retain passed arguments
    argin = true;               % Set flag to indicate arguments were provided
end
...
rejection_file = '';   % Initialize the rejection file variable (used later in data processing)
...
% defines what preprocessing to do 
...
todo.PE              = 0;
todo.statsTF         = 0;
todo.extractLFP      = 0; % 1 event / 2 trial : Extract LFP before making TF
todo.EMG             = 0; % compute EMG instead of TF
...
ica                       = 0;
norm                      = 0;                  % 0 = raw; 1 = z-score normalization, 2 = subtract, 3 = divide, 4 = log(divide), 5 = Baseline par moyenne
...
Artefact_Rejection_Method = 'none';               % 'TraceBrut' , 'TF',  'none'


% --- Condition 1: Time-Frequency (TF)-based Artifact Rejection ---
if strcmp(Artefact_Rejection_Method, 'TF') && ~todo.EMG % reject artefacts TF + emg not processed
    Size_around_event = 0.5;  % Time window (in seconds) around the event to check for artifacts
    Acceptable_Artefacted_Sample_In_Window = 5.7;  % Artifact detection threshold in dB (mean value)
    % Explanation: If the average power in dB within the 0.5s window exceeds 5.7 dB,
    % the segment is considered contaminated by artifacts.

% --- Condition 2: Raw Signal (TraceBrut)-based Artifact Rejection ---
elseif strcmp(Artefact_Rejection_Method, 'TraceBrut') && ~todo.EMG
    Size_around_event = 1;    % Time window (in seconds) around the event for artifact detection
    Acceptable_Artefacted_Sample_In_Window = 120;  % Threshold for raw signal amplitude
    % Explanation: If the raw signal exceeds 120 (likely in microvolts) within the 1s window,
    % the segment is flagged as artifacted. This method focuses on detecting large amplitude deviations.

% --- Condition 3: No Artifact Rejection ---
elseif strcmp(Artefact_Rejection_Method, 'none')
    Size_around_event = 0;    % No window applied since artifact rejection is disabled
    Acceptable_Artefacted_Sample_In_Window = 0;  % No threshold needed
    % Explanation: Artifact rejection is turned off. All data will be processed without any artifact checks.
end
...
% Explanation: When segmenting data by steps, the resting condition is APA,
        % and minimal padding (1 unit) is applied around the data segment.
...
if isunix
    startpath = "/network/iss/pf-marche" ;
...
elseif strcmp(segType, 'trial')
        event    = {'BSL'}; % baseline event
end

complet   = {'FRj_0610','BAg_0496','GAl_000a','DEp_0535','ALb_000a','VIj_000a',...
             'FEp_0536','DRc_000a','DEj_000a','COm_000a','LOp_000a','SOh_0555',...
             'GUg_0634','GIs_0550','SAs_000a','BEm_000a','REa_0526','AUa_0342','FRa_000a'};  % List of patients


% set patients 
% if no argument passed , patients define several time ?
if ~argin
    ...
    subject   = {'SAs_000a','GIs_0550','REa_0526'}


    fprintf(2, ['Bad event list ATTENTION ligne 129 \n'])
event    = {'BSL' } %{'FIX', 'CUE', 'T0', 'T0_EMG', 'FO1', 'FC1', 'FO', 'FC', 'TURN_S', 'TURN_E', 'FOG_S', 'FOG_E'};
    fprintf(2, ['Bad event list ATTENTION ligne 129 \n'])

else
    subject = varargin(1) ;
    if nargin > 1
        event   = varargin{2} ; %%  WHY we don't pick the first patient ?? 
    end
end
...
%MAGIC file name 
FileName = '*_POSTOP_*_GNG_GAIT_*_LFP'; % FileName de quoi 
...
% pourquoi plusieurs normalisations ??
...
if todo.EMG
    for e = 1:numel(event) % number of elements in events
        event{e} = [event{e} '_EMG'];
    end
end
...
% for each patients, je peux choisir ici
for s = 1:numel(subject) %[10 11 13] %13%:numel(subject) %1:6
    
    disp(subject{s}) % afficher nom du patient
    ...
    for r = 1 : numel(RecDir) %r=3
        ...
        if todo.TF || todo.PE || todo.meanTF || todo.statsTF || todo.extractLFP % equals to 1
            %if todo.TF == 1
            ...
            if ((todo.TF == 1 || todo.TF == 3 || todo.TF == 4 || todo.extractLFP) && norm > 0) || todo.meanTF
                ...
                if todo.meanTF && strcmp(segType, 'step')
                    ...
                    MAGIC.batch.step3_R([OutputFileName suff1 '_meanTF_' suff '_BSL.csv'], bslTF, {'BSL'}, protocol, [], 'meanTF', Size_around_event, Acceptable_Artefacted_Sample_In_Window, todo.TF);
                end
...
%                 tic
                ...
                elseif todo.meanTF
                    ...
                    MAGIC.batch.step3_R(csvFile, dataTF, e, protocol, Artefact_Rejection_Method, 'meanTF', Size_around_event, Acceptable_Artefacted_Sample_In_Window, todo.TF);
                end
