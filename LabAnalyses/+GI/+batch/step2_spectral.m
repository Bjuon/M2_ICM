...
function dataTF = step2_spectral(seg, e, norm, Bsl)
global tBlock
global fqStart
global segType
global rest_cond
...
if ~isempty(e)
    ...
    switch e
        case {'T0', 'T0_EMG', 'FO1', 'FC1'}
            temp   = d.where(@(x) strcmp(x.info('trial').segment, 'APA'));
            winBsl = [0.4  0.9 + tBlock] - tBlock/2;
        case {'FO', 'FC'}
            temp   = d.where(@(x) strcmp(x.info('trial').segment, 'step'));
            winBsl = [0.4  0.9 + tBlock] - tBlock/2;
        case {'TURN_S', 'TURN_E'}
            temp   = d.where(@(x) strcmp(x.info('trial').segment, 'turn'));
            winBsl = [0.4  0.9 + tBlock] - tBlock/2;
        case {'FOG_S', 'FOG_E'}
            temp   = d.where(@(x) strcmp(x.info('trial').segment, 'FOG'));
            %winBsl = [-09  -0.1 + tBlock];
            winBsl = [0.4  0.9 + tBlock] - tBlock/2;
        case {'BSL'}
            temp   = d.where(@(x) strcmp(x.info('trial').segment, rest_cond));
...
%     temp = d.toArray();
    ...
    if norm > 0 %~isempty(e) && 
        ...
        switch e
            case {'BSL', 'T0', 'T0_EMG', 'FO1', 'FC1', 'FO', 'FC', 'TURN_S', 'TURN_E', 'FOG_S', 'FOG_E'}
                trials    = arrayfun(@(x) x.info('trial').nTrial, temp);
                for t = 1:numel(trials)
                    med    = temp(t).info('trial').medication;
                    nTrial = temp(t).info('trial').nTrial;
                    cond   = temp(t).info('trial').condition;
                    idx_t  = find((strcmp(Bsl.med, med) & [Bsl.ntrial{:}]' == nTrial & strcmp(Bsl.cond, cond)) == 1); 
                    if isempty(idx_t)
...
%                 bslTFadd = Bsl.TF;
            case ''
                ...
                for t = 1:numel(Bsl.TF)
                    ...
                    cond        = Bsl.cond{t};
                    idx_t       = find((strcmp(Bsl.med, med) & [Bsl.ntrial{:}]' == nTrial & strcmp(Bsl.cond, cond)) == 1);
                    idx_t_temp  = find(arrayfun(@(x) x.info('trial').nTrial == nTrial & strcmp(x.info('trial').medication, med) & strcmp(x.info('trial').condition, cond), temp) == 1);
                    if isempty(idx_t)
                        error ('idx_t is empty')
                    end
        ...
        if numel(TF) ~= numel(bslTFadd)
            error('not the same nimber of trials')
        end
