...
# faire une excpetion pour FOG avec valid pas valid, mais à part
...
# conditions = c('FOGturn', 'FOGall')
# funType = c('median', 'mean')
funType   = c('median') # 'median'
funAllPat = 'median' # 'median'

OutPutFold = paste(datatype, '_Lim_Time', TimeLim, 's', 'Hz_Col', ColLim, sep='')
if (ElecGroup == 1) {
  ...
  for (nor in normtype) {
    for (gp in groups) {
      ...
      for (cond in conditions) {
        ...
        # aggregate values
        if (ElecGroup == 1) {
          temp3 = temp3[, c("Patient", "Medication", "Condition", "Freq", "Event", "value", "times", "grouping", "HEM_o")] # , "side" )]
          temp3 = aggregate( value ~ Patient + Medication + Condition + Event + Freq + times + grouping + HEM_o, temp3, # + side
                             FUN = favg,
                             na.rm = T,
                             na.action = NULL)
        } else if (ElecGroup == 0) {
          temp3 = temp3[, c("Patient", "Medication", "Condition", "Freq", "Event", "value", "times", "Chan_o", "HEM_o" )] # , "side" )]
          temp3 = aggregate( value ~ Patient + Medication + Condition + Event + Freq + times + Chan_o + HEM_o , temp3,  # + side
                             FUN = favg,
                             na.rm = T,
                             na.action = NULL)
        }
