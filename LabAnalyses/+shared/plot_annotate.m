...
function varargout = plot_annotate(self,varargin)
   p = inputParser;
   ...
   p.addParameter('handle_tf',[],@(x) isnumeric(x) || sum(ishandle(x)) == numel(x));
   ...
   if ~isempty(par.process)
       % DEFINING A TF to plot next to the sample process
       obj_tf = copy(par.process);
       if numel(self) ~= numel(obj_tf)
           error('The number if SampleProcess and SpectralProcess must be equal')
       end
   end
   ...
   if isempty(par.handle) || ~ishandle(par.handle)
      ...
      %h = subplot(1,1,1);
      % handle for LFP
      h    = subplot(numel(self(1).labels),2,1:2:2*numel(self(1).labels));
      % handle for tf
      for nobj = 1 : numel(self(1).labels)
        h_tf(nobj) = subplot(numel(self(1).labels),2,2*nobj);
      end
   else
      h = par.handle;
      h_tf = par.handle_tf;
   ...
   if isempty(sh)
      ...
      jsepSlider.StateChangedCallback = ...
         {@(h,e,x) cellfun(@(x)feval(x{:}),x) {{@refreshPlot self h gui.id hsepSlider obj_tf h_tf par}} };
   else
      ...
      f = {@refreshPlot self h gui.id hsepSlider obj_tf h_tf par};
   ...
   % First draw
   refreshPlot(self,h,gui.id,hsepSlider,obj_tf,h_tf,par);
   h.Visible = 'on';
...
function refreshPlot(obj,h,id,hsepSlider,obj_tf,h_tf,par)
   % Extract specific parameters for this ID
   gui = linq(h.UserData).where(@(x) strcmp(x.id,id)).select(@(x) x).toArray;
   ...
   if newdraw
      ...
      %if spectral processs
      if exist('obj_tf', 'var')
          values_tf = obj_tf(ind1).values{1};
          if ~isreal(values_tf)
              values_tf = abs(values_tf);
          end
          t_tf = obj_tf(ind1).times{1} + obj_tf(ind1).tBlock/2;
          f = obj_tf(ind1).f;
          
          for i= 1:numel(lh)
              j = numel(lh) - i + 1;
              if par.log
                  v = 10*log10(values_tf(:,:,i)');
              else
                  v = values_tf(:,:,i)';
              end
              surf(t_tf,f,v,'edgecolor','none','Parent',h_tf(j));
              view(h_tf(j),0,90);
              ylabel(h_tf(j), obj(ind1).labels(i).name)
          end
      end
