...
WhichPC = 'marion'; %'marion'
patientID = {'02_FRASe'}%' 'NGUPh'  'ARDSy' 'LAUTh' 'RAYTh' 'NGUPh' 'ETIAl' 'SALJe' 'RIMLa' 'WARJe' ' DISPi' 'HUSXa'}; %list of patients to analyze
...
switch WhichPC
    case 'marion'
        DataDir = 'C:\Users\marion.albares\Desktop\Marion_tache_GoNoGo\1_data_patients\TOCs\striatum\'; %path where patienstr' data are stored
    case 'pauline'
        DataDir = '/lena13/home_users/users/laviron/Documents/MATLAB/LFP_PANAM/Marion/1_data_patients/TOCs/striatum/'
end


for nindiv = 1:numel(patientID)
    
    Name= patientID{nindiv};
    
    if perOp
        cd([DataDir Name PerOpDir] )
        key= 'Trial';
    else
        cd([DataDir Name PostOpDir] )
        key= 'trial';
    end
    
    %boucle=dir('FRASe_*PRETF2.mat');
    boucle = dir([Name(4:end) '*_2.mat']);
