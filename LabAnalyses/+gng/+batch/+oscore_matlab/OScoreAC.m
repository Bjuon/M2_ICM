...
function [OscScore, OscFreq, AutoCorrelogramWithoutPeak, Spectrum] = ...
    ...
    % GaussianKernel = gaussmf(-3*KernelSTD:3*KernelSTD, [KernelSTD 0]); % alternative gaussian kernel

    % Smoothing the autocorrelogram with the "high" gaussian kernel (mirror signal at edges to reduce border effects)
    PaddedAutoCorrelogram =  [MirroredAutoCorrelogram(end-size(GaussianKernel,2)+1:end), AutoCorrelogram, MirroredAutoCorrelogram(1:size(GaussianKernel,2))];
    HighSmoothedAutoCorrelogram = conv(PaddedAutoCorrelogram, GaussianKernel, 'same');
    ...
    % Smoothing the autocorrelogram with the "low" gaussian kernel (mirror signal at edges to reduce border effects)
    PaddedAutoCorrelogram =  [MirroredAutoCorrelogram(end-size(GaussianKernel,2)+1:end), AutoCorrelogram, MirroredAutoCorrelogram(1:size(GaussianKernel,2))];
    LowSmoothedAutoCorrelogram = conv(PaddedAutoCorrelogram, GaussianKernel, 'same');
    ...
    % Applying Fast Fourier Transform on smoothed correlogram without peak (manual convolution with Blackman window)
