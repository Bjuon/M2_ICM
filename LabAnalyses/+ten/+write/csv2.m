...
for i = 1:numel(dat)
   for j = 2:5
      if ~isnan(dat(i).visit(j).([score 'Off']))
         fprintf(fid,['%s' d],dat(i).id);
         fprintf(fid,['%g' d '%s' d],dat(i).visit(j).([score 'Off']),'Off');
      ...
      if ~isnan(dat(i).visit(j).([score 'On']))
         fprintf(fid,['%s' d],dat(i).id);
         fprintf(fid,['%g' d '%s' d],dat(i).visit(j).([score 'On']),'On');
         fprintf(fid,['%g' d],dat(i).visit(j).monthsReIntervention);
         fprintf(fid,['%g' d],t(j));
         fprintf(fid,['%s' d '%g' d '%g' d '%g' d],...
            dat(i).sex,dat(i).ageAtIntervention,dat(i).ageDebut,dat(i).dureeEvolution2);
