summary.clusterRobust <-
  function (object, VarCov) {
    ...
    coefsY <- cbind("Value" = betas, "Std.Err" = sds, "z-value" = betas / sds, 
                    #"p-value" = 1.96 * pnorm(abs(betas / sds), lower.tail = FALSE))
                    "p-value" = pt(abs(betas / sds), df=df, lower.tail = FALSE))
    ...
    coefsT <- cbind("Value" = gammas, "Std.Err" = sds, "z-value" = gammas / sds,
                    #"p-value" = 1.96 * pnorm(abs(gammas / sds), lower.tail = FALSE))
                    "p-value" = pt(abs(gammas / sds), df=df, lower.tail = FALSE))
