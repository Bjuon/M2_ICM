classdef MAGIC < metadata.Trial
    properties
        ...
        task          % GOc (control), GOi (incertain), NoGO
        condition     % 1st step, steps, return, FOG
        side          % left, right foot
        nStep         % number of step after APA
        isValid       % 0 if GO with omission or NoGo with mouvment initiation
        isFOG         % FOG or not during step or turn or turn during FOG
    ...
    methods
        function self = MAGIC(varargin)
            ...
            p.addParameter('isFOG',[],@(x) isscalar(x));
