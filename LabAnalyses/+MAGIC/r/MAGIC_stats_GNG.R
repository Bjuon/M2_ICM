...
LoadLibrary_and_RSourceFiles = function() {
  library(sp) ; library(tibble) ; library(reshape2) ; library(RColorBrewer) ; library(colorRamps) ; library(vroom) ; library(grDevices) ; 
  library(ggplot2) ; library(svglite) ; library(animation) ; library(FedData) ; library(lme4) ; library(parallel) ; library(plyr) ; library(dplyr) ; 
  library(reshape2) ; library(missMDA) ; library(FactoMineR) ; library(factoextra) ; library(stringr) ; library(performance) ; library(emmeans) ; 
  library(carData) ; library(car) ; library(foreach) ; library(doParallel) ; library(colorspace) ; library(magrittr)
}
...
# GroupingMode         = "Region"
Contacts_of_interest = c("STNversus-exclusif", "inSTN-elargi", "STN-SM-exclusif", "STN-AS-exclusif") # Region + "HotspotFOG","Motor" or grouping + "HighestBeta" or "other" + "All"
GroupingMode         = "other"
...
todo_corr_Comport    = F # To do correlation with comportement
todo_corr_Clinique   = F # To do correlation with clinical scores
...
if (Sys.info()["nodename"] == "UMR-LAU-WP011") {
  ...
  OutputDir = "Z:/03_CartesTF/Stats_GNG"
...
Contact = Contacts_of_interest[1]
  for (Contact in Contacts_of_interest) { 
      ...
      for (ev in events) {
        ...
        print(paste0("Start loop : ", Contact, "-", ev , " !!!!!!!!!!!!!"))
          if (!PlotOnlyNoModel) {
...
ResPar = foreach(Freqpoint = as.numeric(fqStart):100, .combine = 'rbind') %:%   # Execution en parallele, variable de sortie = ResPar 
            foreach(timefreq = firstIncludedTP:lastIncludedTP, .combine = 'rbind') %dopar% {
              ...
              if (!Load_0Comput) {
                ...
                DAT_LFP = DAT_LFP[DAT_LFP$value > 0,]
                ...
                model2   = lme4::lmer(10*log(value,base=10) ~ hemisphere + (1+Task+Medication||Patient/Channel), data = DAT_LFP)
                EMMean  = emmeans(model2, ~  hemisphere)      
                emmean  = data.frame(EMMean)
                emmALL  = data.frame(contrast(EMMean,       
                                              setNames(lapply(1:(nrow(emmean)/2),    function(i) c(1, 1)), 
                                              lapply(1:(nrow(emmean)/2) , function(i) paste0('AllInOne-Model')))))
                ...
                temp.df = data.frame(contrast = "AllInOne-ttest", 
                        ...
                        t.ratio = tresult$statistic, 
                        p.value = tresult$p.value)
                EMMean  = emmeans(model2, ~  hemisphere, at = list(Medication = "OFF"))      
                emmean  = data.frame(EMMean)
                emmAL3  = data.frame(contrast(EMMean,       
                                              setNames(lapply(1:(nrow(emmean)/2),    function(i) c(1, 1)), 
                                              lapply(1:(nrow(emmean)/2) , function(i) paste0('AllInOne-ModelOFF')))))
                emmALL = rbind(emmALL, temp.df, emmAL3)
                ...
                emmean  = data.frame(EMMean)
                emVersus= EMMean %>% pairs() %>% as.data.frame()
               
                # All type of MetaFOG
                emFOG = data.frame(contrast(EMMean,       
                                              setNames(lapply(1:nrow(emmean),    function(i) c(rep(0, i-1), 1, rep(0, nrow(emmean)-i))), # nolint
                                                        apply(       emmean , 1, function(x) paste("FOG", x[1], sep = '_')))))
              ...
              # Put all pvalues in ModelOutput
                ...
                ModelOutput = rbind(emGlobal, emmMED, emmALL, emOIall, emdOIall, emVersus, emFOG, emOItask, emdOItask, emOIside, emdOIside, emside, emdside)
                    ...
                    # Effet fixe :
                    model = lme4::lmer(10*log(value,base=10) ~ Region + Medication*Task + hemisphere*ipsi_contra + (1|Patient/Channel), data = DAT_LFP)
                    ...
                    DAT_LFP$FOG[DAT_LFP$FOG == 'Meta_FOG_0'] = 0
                    DAT_LFP$FOG[DAT_LFP$FOG == 'Meta_FOG_1'] = 1
                    DAT_LFP$FOG[DAT_LFP$FOG == 'Meta_FOG_2'] = 2
                    ...
                    model = lme4::lmer(10*log(value,base=10) ~ Region*FOG + Medication*Task*hemisphere*ipsi_contra + (1|Patient/Channel), data = DAT_LFP)
                        ...
                        emVerFog = EMMean %>% pairs() %>% as.data.frame()
                        
                        for (rown in 1:nrow(emVerFog)) {
                          digitsRow = str_extract_all(emVerFog$contrast[rown], "\\d+")
                          if (digitsRow[[1]][1] != digitsRow[[1]][2]) {
                            if((grepl("AS", emVerFog$contrast[rown]) + grepl("SM", emVerFog$contrast[rown]) + grepl("MidSTN", emVerFog$contrast[rown])) == 2) {
                              emVerFog$contrast[rown] = 'Discard'
                            } else emVerFog$contrast[rown] = paste0('Fog_per_Reg ', emVerFog$contrast[rown])
                          } else emVerFog$contrast[rown] = paste0('Reg_per_Fog ', emVerFog$contrast[rown])
                        }
                        
                        emVerFog %<>% filter(contrast != 'Discard')
        
                          
                    # All effects: 
                    model   = lme4::lmer(10*log(value,base=10) ~ Region*Task + hemisphere*Medication + (1|Patient/Channel), data = DAT_LFP)
                        EMMean   = emmeans(model, ~ Region*Task, at = list(Medication = "OFF"))      
                        emmean   = data.frame(EMMean)
                        emVersus2= EMMean %>% pairs() %>% as.data.frame() %>% mutate(contrast = paste0(contrast, ' TaskxReg'))
                        for (rown in 1:nrow(emVersus2)) {
                          if((grepl("AS", emVersus2$contrast[rown]) + grepl("SM", emVersus2$contrast[rown]) + grepl("MidSTN", emVersus2$contrast[rown])) == 2) {
                            if((grepl("GOc", emVersus2$contrast[rown]) + grepl("GOi", emVersus2$contrast[rown])) == 2) {
                              emVersus2$contrast[rown] = 'Discard'
                            } 
                          }
                        }
                        emVersus2 %<>% filter(contrast != 'Discard')
                        
                        emReg2   = data.frame(contrast(EMMean,       
                                                      setNames(lapply(1:nrow(emmean),    function(i) c(rep(0, i-1), 1, rep(0, nrow(emmean)-i))), # nolint
                                                                apply(       emmean , 1, function(x) paste(x[1],x[2], 'TasketReg', sep = '_')))))
                        
                    rm(ModelOutput)
                    ModelOutput = rbind(emVersus, emReg, emVerOFF, emRegOFF, emNoCont, emVerFog, emVersus2, emReg2)
            ...
            if (todo_Plots) {
              ...
              foreach(MetCondnum = 1:length(unique(pval$MetaCond))) %dopar% { # nolint
                ...
                if (todo_tfmapplot) {
                  ...
                  if (overrideLim) lim  = max(abs(data$Power))
