...
g <- ggplot(loadings, aes(name, forcats::fct_rev(var), fill = -value)) +
  ...
  scale_fill_gradient2(low = "#075AFF", mid = "white", high = "#FF0000") +
  scale_x_discrete(limits = levels(loadings$name),        # <- keeps -RPC5 last
...
control <- lmerControl(calc.derivs = TRUE, optimizer = "nloptwrap",
                       optCtrl = list(algorithm = "NLOPT_LN_BOBYQA"))
# Only RPC1, RPC2 and RPC5
vars <- paste0("RPC.", c(1,2,5))
...
make_blue_plot <- function(comparison_type) {
  if (comparison_type == "FNF") {
    ...
    df_mod     <- df_scores %>%
                  ...
                  mutate(Pat_Foggeur = factor(Pat_Foggeur, levels = c("-","+")))
    title_text <- "Non-Freezers vs. Freezers"
    fname      <- "lmer_varimax_blue_FNF_125.png"
    sig_color  <- "black"
  } else {
    labels     <- factor_labels_fog
    df_mod     <- df_scores %>%
                  ...
                  mutate(Pat_Foggeur = factor(is_FOG, levels = c("-","+")))
    title_text <- "Freezers: noFOG vs. FOG"
    fname      <- "lmer_varimax_blue_FOG_125.png"
    sig_color  <- "navy"
  }

  plots <- list()
  for (i in vars) {
    # Fit LMER and get marginal means
    m      <- lmer(as.formula(paste0(i, " ~ 1 + Pat_Foggeur*Condition + (1|Subject)")),
                   data = df_mod, control = control)
    emm_df <- summary(emmeans(m, ~Pat_Foggeur*Condition), infer=c(TRUE,TRUE)) %>%
              mutate(Factor = paste0(Pat_Foggeur, " ", Condition))
    ...
    # Planned‐comparison stars
    emmp <- summary(emmeans(m, ~Pat_Foggeur*Condition) %>% pairs(),
                    infer=c(TRUE,TRUE), adjust="fdr") %>%
            as_tibble() %>%
            mutate(contrast = str_remove_all(contrast, "[()]+"),
                   p.adj     = p.adjust(p.value, method = "fdr")) %>%
            filter(p.adj < 0.05)  # only significant after FDR
    comparisons <- str_split(emmp$contrast, " - ")
    annotations <- case_when(
      emmp$p.adj < 0.001 ~ "***",
      emmp$p.adj < 0.01  ~ "**",
      TRUE               ~ "*"
