...
## -------------------------------------------------------------------
##  🆕  CODE ADDED / MODIFIED  – copy-paste into your existing script
## -------------------------------------------------------------------

# ... (all previous library calls unchanged) ...
library(stringr)   # <- needed for str_remove_all()
library(purrr)     # <- used for tidy list-columns in significance table
library(ggsignif)   # <- add once (after ggplot2) so geom_signif() is available
...
# ── helper that merges the two original panels into one ─────────────
make_combined_blue_plot <- function() {

  ## 1⃣ data subsets that feed the two models ------------------------
  df_fnf <- df_scores %>%                      # “Non-freezers vs Freezers-(OBS-)”
    filter(Factor %in% c("OFF.-.-", "ON.-.-", "OFF.+.-", "ON.+.-")) %>%
    mutate(group = ifelse(Pat_Foggeur == "-", "FOGNOBS", "FOGOBS-"),
           x_cat = factor(paste(group, Condition),
                          levels = names(factor_labels_combined)))

  df_fog <- df_scores %>%                      # “OBS- vs OBS+”  *inside* freezers
    filter(Factor %in% c("OFF.+.+", "OFF.+.-", "ON.+.-")) %>%
    mutate(group = ifelse(is_FOG == "+", "FOGOBS+", "FOGOBS-"),
           x_cat = factor(paste(group, Condition),
                          levels = names(factor_labels_combined)))
  ...
  for (rpc in vars) {
    ...
    ## 2⃣ inter-group model (black sig. bars) ------------------------
    ...
    sig_fnf <- summary(pairs(emmeans(m_fnf, ~ Pat_Foggeur*Condition)),
                       infer = TRUE, adjust = "fdr") %>%
      as_tibble() %>%
      mutate(p.adj = p.adjust(p.value, "fdr"),
             anno  = case_when(p.adj < .001 ~ "***",
                               p.adj < .01  ~ "**",
                               p.adj < .05  ~ "*",
                               TRUE         ~ "ns")) %>%
      filter(anno != "ns") %>%
      mutate(contrast   = str_remove_all(contrast, "[() ]"),
             comparisons = map(str_split(contrast, "-"), trimws))

    ## 3⃣ intra-group model OBS- vs OBS+ (navy sig. bars) ------------
    ...
    sig_fog <- summary(pairs(emmeans(m_fog, ~ is_FOG*Condition)),
                       infer = TRUE, adjust = "fdr") %>%
      as_tibble() %>%
      mutate(p.adj = p.adjust(p.value, "fdr"),
             anno  = case_when(p.adj < .001 ~ "***",
                               ...
                               TRUE         ~ "ns")) %>%
      filter(anno != "ns",
             str_detect(contrast, "\\+\\sOFF")) %>%      # keep OBS+ OFF comparison
      mutate(contrast   = str_remove_all(contrast, "[() ]"),
             comparisons = map(str_split(contrast, "-"), trimws))

    ## 4⃣ estimates to plot (bind the 4 FNF levels + single OBS+ OFF) --
    emm_plot <- bind_rows(
      emm_fnf %>% filter(x_cat != "FOGOBS+ OFF"),   # from FNF model
      emm_fog %>% filter(x_cat == "FOGOBS+ OFF")    # from FOG model
    ) %>% distinct(x_cat, .keep_all = TRUE)

    ## 5⃣ reference (healthy controls) horizontal turquoise bar -------
    ctl_m <- df_scores %>% filter(Factor == "Sain..") %>%
      pull(!!sym(rpc)) %>% mean()

    ## 6⃣ build single facet -----------------------------------------
    p <- ggplot(emm_plot, aes(x_cat, emmean, group = 1)) +
      ...
      geom_point(colour = "navy", size = 4) +          # ← point colour now navy
      # significance: black for FNF, navy for OBS- vs OBS+
      { if (nrow(sig_fnf) > 0) 
      geom_signif(
        comparisons   = sig_fnf$comparisons,
        annotations   = sig_fnf$anno,
        step_increase = 0.15,
        textsize      = 6,
        colour        = "black"
      )
  ...
  # only add intra-group (OBS+ vs OBS−) asterisks if present
  { if (nrow(sig_fog) > 0)
      geom_signif(
        comparisons   = sig_fog$comparisons,
        annotations   = sig_fog$anno,
        step_increase = 0.30,
        textsize      = 6,
        colour        = "navy"
      )
  } +
      scale_x_discrete(labels = factor_labels_combined,
                       guide  = guide_axis(angle = 90)) +
      ggtitle(switch(rpc,
                     "RPC.1" = "RPC1 : Pace",      "RPC.2" = "RPC2 : Rhythm",
                     "RPC.3" = "RPC3 : Asymmetry", "RPC.4" = "RPC4 : Velocity",
  ...
  ## 7⃣ wrap facets, save & return -----------------------------------
  combined <- wrap_plots(plots, nrow = 1) +
    plot_layout(guides = "collect") +
    plot_annotation(title = "Combined inter-group vs intra-group")
  cowplot::ggsave2(
    here("Figures_MarcheLancee", "lmer_varimax_blue_COMBINED.png"),
