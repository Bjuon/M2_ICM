...
make_combined_blue_plot <- function() {
  ...
  label_map     <- setNames(names(factor_labels_combined), legacy_levels)
  ...
  for (rpc in vars) {
    ...
    emm_plot <- bind_rows(summary(emm_fnf), summary(emm_fog)) %>%
      ...
      mutate(x_cat_new = factor(label_map[as.character(x_cat_legacy)],
    ...
    # — manual “cheat” definitions —
    manual_sig <- list()
    if (rpc == "RPC.1") {
      manual_sig$fnf <- list(pairs = list(c(1,2),c(2,3),c(3,4),c(1,4)),
                             stars = c("***","***","***","**"))
      manual_sig$fog <- list(pairs = list(c(3,5),c(4,5)),
                             stars = c("***","***"))
    } else if (rpc == "RPC.2") {
    ...
    step         <- data_range * 0.08    # 8% step
    buffer       <- data_range * 0.12   # 12% buffer
    tick_down    <- step * 0.3          # 30% of step downwards
    ...
    # — build plot with ticks & text annotations —
    p <- ggplot(emm_plot, aes(x = x_cat_new, y = emmean, group = 1)) +
      ...
      # Black FNF bars + ticks + stars
      unlist(lapply(seq_len(nfn), function(i) {
        x1 <- manual_sig$fnf$pairs[[i]][1]
        x2 <- manual_sig$fnf$pairs[[i]][2]
        y  <- y_fnf[i]
        list(
          ...
          annotate("text", x=(x1+x2)/2, y=y+step*0.1,
                   label=manual_sig$fnf$stars[i],
                   size=7, colour="black")
        )
      }), recursive=FALSE) +
      # Navy FOG bars + ticks + stars
      unlist(lapply(seq_len(nfg), function(i) {
        x1 <- manual_sig$fog$pairs[[i]][1]
        x2 <- manual_sig$fog$pairs[[i]][2]
        y  <- y_fog[i]
        list(
          annotate("segment", x=x1, xend=x2, y=y, yend=y,
                   size=0.5, colour="navy"),
          annotate("segment", x=x1, xend=x1, y=y, yend=y-tick_down,
                   size=0.5, colour="navy"),
          annotate("segment", x=x2, xend=x2, y=y, yend=y-tick_down,
                   size=0.5, colour="navy"),
          annotate("text", x=(x1+x2)/2, y=y+step*0.1,
                   label=manual_sig$fog$stars[i],
                   size=7, colour="navy")
        )
      }), recursive=FALSE) +
      scale_x_discrete(
        labels = factor_labels_combined,
        limits = names(factor_labels_combined),
        guide  = guide_axis(angle = 90)
      ) +
      coord_cartesian(
        ylim = c(data_min * 1.1,
