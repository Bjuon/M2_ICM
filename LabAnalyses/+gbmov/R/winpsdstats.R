...
library(lmerTest)
library(multcomp)
data <- read.csv("/Users/brian/Documents/MATLAB/test3.txt")
data <- within(data, {
...
lme0 = lmer(f13 ~ 1 + CONDITION*CHANNEL + SIDE + UPDRSIV + DUREE_LDOPA + (1|PATIENTID/SIDE/CHANNEL),data=data,REML=FALSE)

fname = paste0("f",4:40)
base = "~ 1 + CONDITION*CHANNEL + UPDRSIII + UPDRSIV + CONDITION*EQUIVDOPA + (1|PATIENTID/SIDE/CHANNEL)"
