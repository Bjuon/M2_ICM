...
function [data] = LoadExperimentReach1(matfile,lfpfile)
   ...
   if isempty(str2num(matfile(end)))
      ...
      if numel(matfile) == numel(lfpfile)
         ...
         for i = 1:numel(matfile)
            temp = LoadExperimentReach1(matfile{i},lfpfile{i});
...
% side.
events = sig.detectEvents(signal.data{1},1/signal.fs);
...
for i = 1:min(numel(s),numel(info))
   ...
   data(i) = Segment('info',temp,'process',s(i),'labels','lfp');
end
