...
addParamValue(par,'frequency_range',[8 35],@isnumeric);
...
for i = 1:numel(fnames)
  ...
  % meanPdB = 10*log10(nanmean(P(:,ind,:),3));
   if par.normalize
      ...
      %meanPdB = bsxfun(@minus,meanPdB,mean(meanPdB(ind2,:)));
   end

   [peakLoc,peakMag,peakDetected,truepeak,bandmax,bandavg] = psdpeak(10*log10(meanP),f,par.frequency_range);
   out(1).Fs = Fs;
   out(1).origFs = origFs;
