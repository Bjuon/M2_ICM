...
% [p,trial,ep,hdr] = monk.load.session(plexonfname,eventidefname);

% [p,trial,ep,hdr] = monk.load.session('Flocky_GNG_08022018_S.pl2','Flocky_GNG_data-2018-02-08_04-10-35.txt');
% monk.plot.GNG(hdr,p,trial,ep,'alignTo','Target')
%
function [p,trial,ep,hdr] = session(plexonfname,eventidefname,varargin)
p = inputParser;
...
if nPlexon ~= nEventIDE
   ...
   if nPlexon < nEventIDE
      fprintf('%g Plexon trials, %g EventIDE trials\n',nPlexon,nEventIDE);
