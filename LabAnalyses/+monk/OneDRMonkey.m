...
classdef OneDRMonkey < metadata.Trial
   ...
   methods
      ...
      function plotBasic(self,ti)
         ...
         h = subplot(4,2,1);
         self.plotMetricByTrial(h,'RT',{'T0R0','T0R1'});
         h = subplot(4,2,2);
         self.plotMetricByTrial(h,'RT',{'T1R0','T1R1'});
         
         h = subplot(4,2,3);
         self.plotMetricByDuration(h,'Fix+Target','RT',{'T0R0','T0R1'});
         axis tight;
         h = subplot(4,2,4);
         self.plotMetricByDuration(h,'Fix+Target','RT',{'T1R0','T1R1'});
         axis tight;
      ...
      function plotMetricByTrial(self,h,metric,uCondition,color,ms)
         if nargin < 2
            h = subplot(1,1,1);
         elseif isempty(h)
            h = subplot(1,1,1);
         ...
         for i = 1:numel(uCondition)
            ...
            if any(ind2)
               ...
               t = text(ex,min(max(metric(ind2)),1000),...
         ...
         ylim = get(gca,'ylim');
         axis([trial(1) trial(end) ylim(1) min(ylim(2),1000)]);
      ...
      function plotMetricByDuration(self,h,event,metric,uCondition)
         if nargin < 2
            h = subplot(1,1,1);
         elseif isempty(h)
            h = subplot(1,1,1);
