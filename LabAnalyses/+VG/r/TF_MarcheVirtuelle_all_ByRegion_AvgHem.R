...
for (favg in funType) {
  for (nor in normtype) {
    for (gp in groups) {
      ...
      if (plot_allFq == 1) {
        OutputPathAll = paste(OutputPathAll, 'plot_allFq', sep='_')
        OutputPathInd = paste(OutputPathInd, 'plot_allFq', sep='_')
      }
      ...
      remove(temp2)
      temp2 <-
        read.table(paste('temp_TF', '_', gp, '_', nor, favg, '.csv', sep = ""))
      ...
      temp2 = temp2[, c("Patient", "Medication", "Condition", "IsDoor", "Freq", "Event", "value", "times", "grouping")]
      temp2 = aggregate( value ~ Patient + Medication + Condition + IsDoor + Event + Freq + times + grouping, temp2,
                         FUN = favg,
                         na.rm = T,
                         na.action = NULL
      )
      ...
      for (cond in conditions) {
        ...
        if (cond == 'marche' | cond == 'tapis') {
          ...
          ## Delta
          temp3       = temp2[temp2$Condition == 'marche', c('Patient', 'Medication', 'Event', 'IsDoor', 'Freq', 'times', 'grouping')] # 'Chan_o', 'HEM_o')]
        ...
        for (d in doorCond) {
          if (d == 'noDoor') {
            ...
            temp4       = temp3[temp3$IsDoor == 1, c('Patient', 'Medication', 'Event', 'Freq', 'times', 'grouping')] # 'Chan_o', 'HEM_o')]
            ...
            for (s in listnameSubj) {
              ...
              if (plot_allFq == 0) {
                ...
                ggplot(temp4_ind, aes(x = times, y = Freq, fill = value)) +
                  ...
                  ggtitle(paste(s, nor, cond, d, favg, 'AvgHem', sep = "_")) +
                  theme(plot.title = element_text(hjust = 0.5))
                
                ## sauvegarde des graphes
                ggsave(paste(OutputPathInd, '/', paste(s, nor, cond, d, favg, 'AvgHem', sep = "_"), '.png', sep = ""),
                       width = 6, height = 8)
                ...
                ## sauvegarde des graphes
                ggsave(paste(OutputPathInd, '/', paste(s, nor, cond, favg, 'AvgHem', 'verylowFq', sep = "_"), '.png', sep = ""),
                ...
                ggplot(temp4_ind_fq1, aes(x = times, y= value, color = as.factor(Freq))) +
                  ...
                  theme(plot.title = element_text(hjust = 0.5))
                ## sauvegarde des graphes
                ggsave(paste(OutputPathInd, '/', paste(s, nor, cond, favg, 'AvgHem', 'lowFq', sep = "_"), '.png', sep = ""),
                       width = 32, height =18, units = "cm")
                remove(temp4_ind_fq1)              
                ...
                ggplot(temp4_ind_fq1, aes(x = times, y= value, color = as.factor(Freq))) +
                  ...
                  theme(plot.title = element_text(hjust = 0.5))
                ## sauvegarde des graphes
                ggsave(paste(OutputPathInd, '/', paste(s, nor, cond, favg, 'AvgHem', 'HighFq', sep = "_"), '.png', sep = ""),
                       width = 32, height =18, units = "cm")
                remove(temp4_ind_fq1)
                ...
                ggplot(temp4_ind_fq1, aes(x = times, y= value, color = as.factor(Freq))) +
                  ...
                  theme(plot.title = element_text(hjust = 0.5))
                ## sauvegarde des graphes
                ggsave(paste(OutputPathInd, '/', paste(s, nor, cond, favg, 'AvgHem', 'veryHighFq', sep = "_"), '.png', sep = ""),
                       width = 32, height =18, units = "cm")
                remove(temp4_ind_fq1)
              }
            }
          ...
          if (PlotAll == 1) {
            ...
            if (plot_allFq == 0) {
              ...
              ggplot(temp4, aes(x = times, y = Freq, fill = value)) +
                ...
                geom_vline(xintercept = 0, size = .1) + theme_classic() +
                facet_grid(grouping ~ Medication + Event, drop = TRUE, scales = "free_x") +
                ggtitle(paste(gp, nor, cond, d, favg, funAllPat, 'AvgHem', sep = "_")) +
                theme(plot.title = element_text(hjust = 0.5))
              
              ## sauvegarde des graphes
              ggsave(paste(OutputPathAll, '/', paste(gp, nor, cond, d, favg, funAllPat, 'AvgHem', sep = "_"), '.png',sep = ""),
                     width = 6, height = 8)
              ...
              ggplot(temp4_fq1, aes(x = times, y= value, color = as.factor(Freq))) +
                ...
                theme(plot.title = element_text(hjust = 0.5))
              ## sauvegarde des graphes
              ggsave(paste(OutputPathAll, '/', paste(gp, nor, cond, favg, 'AvgHem', 'verylowFq', sep = "_"), '.png', sep = ""),
                     width = 32, height =18, units = "cm")
              remove(temp4_ind_fq1)
              ...
              ggplot(temp4_fq1, aes(x = times, y= value, color = as.factor(Freq))) +
                ...
                theme(plot.title = element_text(hjust = 0.5))
              ## sauvegarde des graphes
              ggsave(paste(OutputPathAll, '/', paste(gp, nor, cond, favg, 'AvgHem', 'lowFq', sep = "_"), '.png', sep = ""),
                     width = 32, height =18, units = "cm")
              remove(temp4_ind_fq1)              
              ...
              ggplot(temp4_fq1, aes(x = times, y= value, color = as.factor(Freq))) +
                ...
                theme(plot.title = element_text(hjust = 0.5))
              ## sauvegarde des graphes
              ggsave(paste(OutputPathAll, '/', paste(gp, nor, cond, favg, 'AvgHem', 'HighFq', sep = "_"), '.png', sep = ""),
                     width = 32, height =18, units = "cm")
              remove(temp4_ind_fq1)
              ...
              ggplot(temp4_fq1, aes(x = times, y= value, color = as.factor(Freq))) +
                ...
                theme(plot.title = element_text(hjust = 0.5))
              ## sauvegarde des graphes
              ggsave(paste(OutputPathAll, '/', paste(gp, nor, cond, favg, 'AvgHem', 'veryHighFq', sep = "_"), '.png', sep = ""),
                     width = 32, height =18, units = "cm")
              remove(temp4_fq1)
            }            
