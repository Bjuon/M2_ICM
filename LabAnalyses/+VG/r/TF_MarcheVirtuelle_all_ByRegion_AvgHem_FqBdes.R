...
plot_allFq = 1 # plot TF with one line per fq and not in TF

# PRECISE nomarlization types
# normtype = c('RAW', 'zNOR', 'sNOR', 'dNOR')
normtype = c('zNOR')
datatype = 'FqBdes' #meanTF' #'PE' # TF
# PRECISE GROUPS
# groups   = c('STN', 'PPN')
groups   = c('STN')
# SELECT EVENTS
...
for (favg in funType) {
  for (nor in normtype) {
    for (gp in groups) {
      ...
      remove(temp2)
      temp2 <-
        read.table(paste('temp', '_', datatype,  '_', gp, '_', nor, favg, '.csv', sep = ""))
      ...
      for (cond in conditions) {
        ...
        temp3$Freq <- factor(temp3$Freq, levels = c("1-3", "4-7", "8-12", "13-20", "21-35", "36-60", "61-80"))
        ...
        for (d in doorCond) {
          ...
          if (PLotInd == 1) {
            for (s in listnameSubj) {
              ...
              ggplot(temp4_ind, aes(x = times, y= value, color = as.factor(Freq))) +
                ...
                ggtitle(paste(s, 'FqBdes', favg , sep = "_")) +
                theme(plot.title = element_text(hjust = 0.5))
              
              ## sauvegarde des graphes
              ggsave(paste(OutputPathInd, '/', paste(s, nor, cond, d, favg, 'AvgHem', 'FqBdes', sep = "_"), '.png', sep = ""),
                     width = 6, height = 8)
          ...
          if (PlotAll == 1) {
            ...
            ggplot(temp4, aes(x = times, y= value, color = as.factor(Freq))) +
              ...
              theme_classic() + 
              facet_grid(grouping ~ Medication + Event, drop = TRUE,scales = "free_x") +
              ggtitle(paste(gp, 'FqBdes', favg , sep = "_")) +
              theme(plot.title = element_text(hjust = 0.5))
            
            ## sauvegarde des graphes
            ggsave(paste(OutputPathAll, '/', paste(gp, nor, cond, d, favg, 'AvgHem', 'FqBdes', sep = "_"), '.png', sep = ""),
