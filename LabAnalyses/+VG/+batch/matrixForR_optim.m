...
MForR{1,5}      = 'IsDoor'; %'Instruction'; %
MForR{1,6}      = 'DistDoor'; % distance to door
...
MForR{1,14}     = 'DoorCond'; % ex10
MForR{1,15}     = 'quality'; %'Instruction'; %
...
%     end
    ...
    for tr = 1:Ntrial
        ...
        MForR(index_tr+1:index_tr+NLine_trial,5)        = {trials(tr).info('trial').isDoor}; %instruction};
        MForR(index_tr+1:index_tr+NLine_trial,6)        = {trials(tr).info('trial').distDoor}; 
        ...
        MForR(index_tr+1:index_tr+NLine_trial,13)       = e ;
        MForR(index_tr+1:index_tr+NLine_trial,14)       = {trials(tr).info('trial').DoorCond};
        MForR(index_tr+1:index_tr+NLine_trial,15)       = {trials(tr).info('trial').quality}; 
            ...
            for ch=1:numel(Chan{1,tr})
                ...
                if ~isempty(reject_table)
                    idx_quality = find(strcmp(reject_table.patient, Patients{pat}) & ...
                        ...
                        strcmp([reject_table.run{:}]', num2str(trials(tr).info('trial').run)) & ...
                        strcmp([reject_table.nTrial{:}]', num2str(trials(tr).info('trial').nTrial)) & ...
                        strcmp([reject_table.Channel{:}]', Chan{1,tr}{ch}) & ...
                        strcmp([reject_table.Segment{:}]', win_name) == 1,1);
                    if ~isempty(idx_quality)
                        if numel(idx_quality) > 1
                            error('found several idx for artefact rejection')
                        end
                        %reject_table(idx_quality,:)
%                         MForR(index_tr+1+(ch-1)*numel(F):index_tr+ch*numel(F),5)    = num2cell(0);
                        MForR(index_tr+1+(ch-1)*numel(F):index_tr+ch*numel(F),15)    = num2cell(0);
                    end
                end
