...
f = 1;
logfile = dir(fullfile(LogPath, [files.name(1:end-9) 'LOG.csv']));
...
if strcmp(trig.labels.name, 'triggerMUA')
    ...
    %read log file
    [ ~, trig_log] = VG.load.read_log(fullfile(files(f).folder, files(f).name), RecID);
    ...
    %read log file
%     [ ~, trig_log] = VG.load.read_log(fullfile(files(f).folder, [files(f).name(1:end-9) 'LOG.csv']), RecID);
    [ ~, trig_log] = VG.load.read_log(fullfile(logfile.folder, logfile.name), RecID);
    FigName = strtok(files(f).name, '.');
end

% add exception if any to trig_LFP
[trig_LFP, maxDiffLim]  = VG.batch.triggers_exceptions(trig_LFP, trig_log, FigName); %strtok(files(f).name, '.'));
