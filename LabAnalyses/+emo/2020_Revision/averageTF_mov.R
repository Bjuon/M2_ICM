...
for (fstep in f) {
  print(fstep)
  temp = loadTFchunk(f=fstep,epoch="mov")
  data = temp[[1]]
  TF = temp[[2]]
  t = temp[[3]]
  
  ind = which((data$Treat=="TOC")&(data$Cond=="motor"))
  temp = colMeans(TF[ind,])
  TF_TOC = rbind(TF_TOC,temp)
  
  ind = which((data$Treat=="OFF")&(data$Cond=="motor"))
  temp = colMeans(TF[ind,])
  TF_OFF = rbind(TF_OFF,temp)
  
  ind = which((data$Treat=="ON")&(data$Cond=="motor"))
  temp = colMeans(TF[ind,])
  TF_ON = rbind(TF_ON,temp)
}

save(t,f,TF_TOC,TF_OFF,TF_ON,file='averageTFbyTreat_mov.RData')
