...
pTOC = plottf(df,clim=c(-.05,.05),plotcontour = T,logfreq=logfreq,breaks = breaks)

df = res_X[res_X$Treat=="OFF",]
df$tstep = 1:89
df$estimate = df$X.trend
pOFF = plottf(df,clim=c(-.05,.05),plotcontour = T,logfreq=logfreq,breaks = breaks)

df = res_X[res_X$Treat=="ON",]
df$tstep = 1:89
df$estimate = df$X.trend
pON = plottf(df,clim=c(-.05,.05),plotcontour = T,logfreq=logfreq,breaks = breaks)
